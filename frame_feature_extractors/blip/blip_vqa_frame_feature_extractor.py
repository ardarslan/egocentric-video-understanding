...
class BLIPVQAFrameFeatureExtractor(FrameFeatureExtractor):
    def __init__(self, args):
        ...
        self.model = blip_vqa(pretrained=self.args.blip_vqa_model_file_path, image_size=self.image_size, vit="base")
        self.model.eval()
    ...
    def predictor_function(self, frame_indices_batch: List[int], frames_batch: List[np.array]):
        ...
        with torch.no_grad():
            for question in self.questions:
                answers = self.model(preprocessed_frames_batch, question, train=False, inference="generate")
