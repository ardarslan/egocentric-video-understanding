...
with open(
    ...
    clip_id_frame_id_ground_truth_labels_mapping = pickle.load(reader)[
...
for (
    clip_id,
    frame_id_ground_truth_labels_mapping,
) in clip_id_frame_id_ground_truth_labels_mapping.items():
    if (
        list(clip_id_frame_id_ground_truth_labels_mapping[clip_id][0])[0]
        == "no_annotation"
    ):
        continue
    ...
    for frame_id, ground_truth_labels in frame_id_ground_truth_labels_mapping.items():
        ...
        for ground_truth_label in ground_truth_labels:
            clip_id_frame_id_ground_truth_label_indices_mapping[clip_id][
                frame_id
            ].append(distinct_ground_truth_labels.index(ground_truth_label))
del clip_id_frame_id_ground_truth_labels_mapping


def evaluate_predictions(
    clip_id_frame_id_predicted_label_indices_mapping: Dict[str, Dict[str, List[str]]],
    clip_id_frame_id_ground_truth_label_indices_mapping: Dict[
    ...
    for clip_id, frame_id_ground_truth_label_indices_mapping in tqdm(
        list(clip_id_frame_id_ground_truth_label_indices_mapping.items())
    ):
        for (
            frame_id,
            ground_truth_label_indices,
        ) in frame_id_ground_truth_label_indices_mapping.items():
            predicted_label_indices = clip_id_frame_id_predicted_label_indices_mapping[
                clip_id
            ][int(frame_id // 6 * 6)]
            predicted_labels_one_hot_vector = np.zeros(
                len(distinct_ground_truth_labels)
            )
            ground_truth_labels_one_hot_vector = np.zeros(
                len(distinct_ground_truth_labels)
            )
            ...
            ground_truth_label_one_hot_vectors.append(
                ground_truth_labels_one_hot_vector
            )
    ...
    weighted_f1 = f1_score(
        y_true=ground_truth_label_one_hot_vectors,
        y_pred=predicted_label_one_hot_vectors,
        average="weighted",
        zero_division=0,
    )
...
def nontemporal_dictionary_matching_for_given_clip(
    ...
    label_verb_noun_tools_mapping: Dict[str, List[Tuple[str, str, str]]],
):
    ...
    for (
        ...
        for label_index in range(len(distinct_ground_truth_labels)):
            if label_index == 0:
                ...
                if (
                    label_index
                    not in frame_id_predicted_label_indices_and_scores[frame_id].keys()
                ):
                    frame_id_predicted_label_indices_and_scores[frame_id][
                        label_index
                    ] = 0.0
...
def nontemporal_select_labels_with_maximum_score_higher_than_threshold(
    nontemporal_clip_id_frame_id_predicted_label_indices_and_scores: Dict[
        str, Dict[str, List[float]]
    ],
    threshold: float,
):
    nontemporal_clip_id_frame_id_predicted_label_indices = dict()
    for clip_id, frame_id_predicted_label_indices_and_scores in tqdm(
        nontemporal_clip_id_frame_id_predicted_label_indices_and_scores.items()
    ):
        ...
        for (
            ...
            for (
                ...
                score,
            ) in predicted_label_indices_and_scores.items():
                if score >= threshold:
                    labels_with_maximum_score_higher_than_a_threshold.append(
                        predicted_label_index
                    )
            nontemporal_clip_id_frame_id_predicted_label_indices[clip_id][
                frame_id
            ] = labels_with_maximum_score_higher_than_a_threshold
...
for (
    clip_id,
    frame_id_ground_truth_label_indices_mapping,
) in clip_id_frame_id_ground_truth_label_indices_mapping.items():
    for frame_id, label_indices in frame_id_ground_truth_label_indices_mapping.items():
        blip2_question_answer_verb_noun_tool_mapping = (
            clip_id_frame_id_blip2_verb_noun_tool_pair_mapping[clip_id][
                int((frame_id // 6) * 6)
            ]
        )
        for label_index in label_indices:
            for dictionary in [
                label_verb_noun_tool_match_ratio_mapping,
                label_verb_noun_match_ratio_mapping,
                label_verb_tool_match_ratio_mapping,
                label_verb_match_ratio_mapping,
                label_verb_noun_tool_count_mapping,
            ]:
            ...
            if label == "background":
                for (
                    blip2_question,
                    blip2_answer_verb_noun_tools,
                ) in blip2_question_answer_verb_noun_tool_mapping.items():
                ...
                for (
                    ...
                    for blip2_verb_noun_tool in blip2_verb_noun_tools:
                        ...
                        for label_verb_noun_tool in label_verb_noun_tools:
                            ...
                            if (
                                blip2_verb == label_verb
                                and blip2_noun == label_noun
                                and blip2_tool == label_tool
                            ):
                                label_verb_noun_tool_match_ratio_mapping[
                                    label_index
                                ] += 1
...
for dictionary in [
    ...
    for label_index in dictionary.keys():
        dictionary[label_index] = dictionary[label_index] / float(
            label_verb_noun_tool_count_mapping[label_index]
        )
...
with open(
    os.path.join(
        os.environ["SCRATCH"], "ego4d_data/v2/analysis_data", "match_ratios.pickle"
    ),
    "wb",
) as writer:
    pickle.dump(
...
with open(
    ...
    clip_id_frame_id_asl_predicted_label_indices_and_scores_mapping = pickle.load(
        reader
    )

for threshold in [0.05, 0.10, 0.25, 0.5, 0.75, 1.0]:
    nontemporal_selected_labels_with_maximum_score_higher_than_threshold = nontemporal_select_labels_with_maximum_score_higher_than_threshold(
        nontemporal_clip_id_frame_id_predicted_label_indices_and_scores=clip_id_frame_id_asl_predicted_label_indices_and_scores_mapping,
        threshold=threshold,
    )
    weighted_f1, weighted_precision, weighted_recall = evaluate_predictions(
        clip_id_frame_id_predicted_label_indices_mapping=nontemporal_selected_labels_with_maximum_score_higher_than_threshold,
        clip_id_frame_id_ground_truth_label_indices_mapping=clip_id_frame_id_ground_truth_label_indices_mapping,
    )
    print(
        f"ASL Ego4D Baseline | Threshold: {np.round(threshold, 2)} | Weighted F1 Score: {np.round(weighted_f1, 2)} | Weighted Precision: {np.round(weighted_precision, 2)} | Weighted Recall: {np.round(weighted_recall, 2)}"
    )


# BLIP2 Dictionary Mapping

nontemporal_dictionary_matching_clip_id_frame_id_predicted_label_indices_and_scores = dict(
    pqdm(
        [
            {
                "clip_id": clip_id,
                "frame_id_blip2_question_answer_verb_noun_tool_pairs_mapping": frame_id_blip2_question_answer_verb_noun_tool_pairs_mapping,
                "label_verb_noun_tools_mapping": label_verb_noun_tools_mapping,
            }
            for clip_id, frame_id_blip2_question_answer_verb_noun_tool_pairs_mapping in clip_id_frame_id_blip2_verb_noun_tool_pair_mapping.items()
        ],
...
with open(
    os.path.join(
        os.environ["SCRATCH"],
        "ego4d_data/v2/analysis_data",
        "nontemporal_dictionary_matching_clip_id_frame_id_predicted_label_indices_and_scores.pickle",
    ),
    "wb",
) as writer:
    pickle.dump(
        nontemporal_dictionary_matching_clip_id_frame_id_predicted_label_indices_and_scores,
        writer,
    )

for threshold in [0.05, 0.10, 0.25, 0.5, 0.75, 1.0]:
    nontemporal_selected_labels_with_maximum_score_higher_than_threshold = nontemporal_select_labels_with_maximum_score_higher_than_threshold(
        nontemporal_clip_id_frame_id_predicted_label_indices_and_scores=nontemporal_dictionary_matching_clip_id_frame_id_predicted_label_indices_and_scores,
        threshold=threshold,
    )
    weighted_f1, weighted_precision, weighted_recall = evaluate_predictions(
        clip_id_frame_id_predicted_label_indices_mapping=nontemporal_selected_labels_with_maximum_score_higher_than_threshold,
    ...
    print(
        f"BLIP2 Dictionary Matching | Threshold: {np.round(threshold, 2)} | Weighted F1 Score: {np.round(weighted_f1, 2)} | Weighted Precision: {np.round(weighted_precision, 2)} | Weighted Recall: {np.round(weighted_recall, 2)}"
    )
...
for clip_id, frame_id_blip2_verb_noun_tool_pair_mapping in tqdm(
    list(clip_id_frame_id_blip2_verb_noun_tool_pair_mapping.items())
):
    nontemporal_sbert_embedding_clip_id_frame_id_predicted_label_indices_and_scores[
        ...
        clip_id=clip_id,
        frame_id_blip2_question_answer_verb_noun_tool_pairs_mapping=frame_id_blip2_verb_noun_tool_pair_mapping,
        label_verb_noun_tools_mapping=label_verb_noun_tools_mapping,
    )[
        1
    ]
...
with open(
    os.path.join(
        os.environ["SCRATCH"],
        "ego4d_data/v2/analysis_data",
        "nontemporal_sbert_embedding_clip_id_frame_id_predicted_label_indices_and_scores.pickle",
    ),
    "wb",
) as writer:
    pickle.dump(
...
for threshold in [0.05, 0.10, 0.25, 0.5, 0.75, 1.0]:
    nontemporal_selected_labels_with_maximum_score_higher_than_threshold = nontemporal_select_labels_with_maximum_score_higher_than_threshold(
        nontemporal_clip_id_frame_id_predicted_label_indices_and_scores=nontemporal_sbert_embedding_clip_id_frame_id_predicted_label_indices_and_scores,
        threshold=threshold,
    )
    weighted_f1, weighted_precision, weighted_recall = evaluate_predictions(
        clip_id_frame_id_predicted_label_indices_mapping=nontemporal_selected_labels_with_maximum_score_higher_than_threshold,
    ...
    print(
        f"BLIP2 SBERT Embedding | Threshold: {np.round(threshold, 2)} | Weighted F1 Score: {np.round(weighted_f1, 2)} | Weighted Precision: {np.round(weighted_precision, 2)} | Weighted Recall: {np.round(weighted_recall, 2)}"
    )
