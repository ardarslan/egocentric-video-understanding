...
if __name__ == "__main__":
    ...
    parser.add_argument("--num_activations", default=3, type=int)
    ...
    for epoch in range(args.num_epochs):
        ...
        for batch in train_data_loader:
            ...
            train_ys.extend(batch["y"].ravel().tolist())
        ...
        train_ys_numpy = train_ys.numpy()
        train_yhats_argmaxed = np.argmax(train_yhats.numpy(), axis=1)
        train_loss = torch.nn.CrossEntropyLoss()(train_yhats, train_ys).item()
        train_bmac = balanced_accuracy_score(train_ys_numpy, train_yhats_argmaxed)
        train_f1 = f1_score(train_ys_numpy, train_yhats_argmaxed, average="weighted")
        ...
        for batch in val_data_loader:
            with torch.no_grad():
                yhat = model(batch["X"].to(args.device))
                y = batch["y"].to(args.device)
                val_ys.extend(batch["y"].ravel().tolist())
                val_yhats.append(yhat.detach().cpu())
        val_ys = torch.tensor(val_ys)
        val_yhats = torch.vstack(val_yhats)
        val_loss = torch.nn.CrossEntropyLoss()(val_yhats, val_ys).item()
        val_ys_numpy = val_ys.numpy()
        val_yhats_argmaxed = np.argmax(val_yhats.numpy(), axis=1)
        val_bmac = balanced_accuracy_score(val_ys_numpy, val_yhats_argmaxed)
        val_f1 = f1_score(val_ys_numpy, val_yhats_argmaxed, average="weighted")
