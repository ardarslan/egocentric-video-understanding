...
def valid_performance(args):
    ...
    if ".pth.tar" in args.checkpoint:
        assert os.path.isfile(args.checkpoint), "CKPT file does not exist!"
        ckpt_file = args.checkpoint
    else:
        assert os.path.isdir(args.checkpoint), "CKPT file folder does not exist!"
        ckpt_file_list = sorted(glob.glob(os.path.join(args.checkpoint, "*.pth.tar")))
    ...
    suffix_name = args.checkpoint.split("/")[-1]
