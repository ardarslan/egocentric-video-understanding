...
def load_gt_seg_from_json(
    json_file, split=None, label="label_id", label_offset=0, debug_video_id=None
):
    # load json file
    ...
    gt_base = pd.DataFrame(
        {"video-id": vids, "t-start": starts, "t-end": stops, "label": labels}
    )
...
class ANETdetection(object):
    ...
    def evaluate(self, preds, verbose=True):
        ...
        if verbose:
            # print the results
            print(
                "[RESULTS] Action detection results on {:s}.".format(self.dataset_name)
            )
            block = ""
            for tiou, tiou_mAP in zip(self.tiou_thresholds, mAP):
                block += "\n|tIoU = {:.2f}: mAP = {:.2f} (%)".format(
                    tiou, tiou_mAP * 100
                )
            print(block)
...
def compute_average_precision_detection(
    ground_truth, prediction, tiou_thresholds=np.linspace(0.1, 0.5, 5)
):
