...
if __name__ == "__main__":
    ...
    slowfast_clip_uids = [
        feature_file_name.split(".")[0]
        for feature_file_name in os.listdir(args.slowfast_features_folder_path)
    ]
    clip_uids = frozenset(set(annotations_clip_uids).intersection(slowfast_clip_uids))

    for folder_path in [
        args.clips_folder_path,
        args.egovlp_egonce_features_folder_path,
        args.internvideo_features_folder_path,
        args.omnivore_features_folder_path,
        args.slowfast_features_folder_path,
    ]:
    ...
    with open(
        args.updated_annotations_json_file_path, "w"
    ) as updated_annotations_json_file:
        updated_annotations_json_dict = dict(
            [(clip_uid, annotations_json_dict[clip_uid]) for clip_uid in clip_uids]
        )
