...
x_data = [
    [1] * int(cap.get(cv2.CAP_PROP_FRAME_COUNT)),
]

y_data = [[2] * int(cap.get(cv2.CAP_PROP_FRAME_COUNT))]
...
fig = px.bar(
    ...
    y="y",
    color="z",
    color_discrete_map={"some_group": "red", "some_other_group": "green"},
)

fig = go.Figure()

for current_frame_id in range(cv2.CAP_PROP_FRAME_COUNT):
    current_action_category = frame_id_ground_truth_action_instances_mapping[
        current_frame_id
    ][0]
    for xd, yd in zip(x_data, y_data):
        fig.add_trace(
            go.Bar(
                x=[xd[current_frame_id]],
                y=[yd[current_frame_id]],
                orientation="h",
                marker=dict(
                    color=action_category_color_mapping[current_action_category],
                    line=dict(color="rgb(255, 255, 255)", width=1),
                ),
            ),
            row=1,
            col=cv2.CAP_PROP_FRAME_COUNT,
        )
