...
def blip2_dictionary_matching_for_given_clip(
    ...
    for (
        ...
        for blip2_question_index, blip2_answer_verb_noun_tool_pairs in blip2_question_answer_verb_noun_tool_pairs_mapping.items():
            frame_id_predicted_label_indices_and_scores_dictionary_matching[frame_id][blip2_question_index] = dict()
            for label_index in range(len(distinct_ground_truth_labels) + 1):
                if label_index == len(distinct_ground_truth_labels):
                    ...
                    if len(blip2_verb_noun_tool_pairs) == 0:
                        ...
                        if label_index not in frame_id_predicted_label_indices_and_scores_dictionary_matching[frame_id][blip2_question_index].keys():
                            frame_id_predicted_label_indices_and_scores_dictionary_matching[frame_id][blip2_question_index][label_index] = []
                        frame_id_predicted_label_indices_and_scores_dictionary_matching[frame_id][blip2_question_index][label_index].append((match_type, 1.00))
                else:
                    ...
                    for label_verb_noun_tool in label_verb_noun_tools:
                        ...
                        for blip2_verb_noun_tool_pair in blip2_verb_noun_tool_pairs:
                            ...
                            if label_verb == blip2_verb and label_noun == blip2_noun and label_tool == blip2_tool:
                                ...
                                if label_index not in frame_id_predicted_label_indices_and_scores_dictionary_matching[frame_id][blip2_question_index].keys():
                                    frame_id_predicted_label_indices_and_scores_dictionary_matching[frame_id][blip2_question_index][label_index] = []
                                frame_id_predicted_label_indices_and_scores_dictionary_matching[frame_id][blip2_question_index][label_index].append((match_type, 0.80))
                            elif label_verb == blip2_verb and label_tool == blip2_tool:
                                match_type = constants.BLIP2_VERB_TOOL_LABEL_VERB_TOOL
                                if label_index not in frame_id_predicted_label_indices_and_scores_dictionary_matching[frame_id][blip2_question_index].keys():
                                    frame_id_predicted_label_indices_and_scores_dictionary_matching[frame_id][blip2_question_index][label_index] = []
                                frame_id_predicted_label_indices_and_scores_dictionary_matching[frame_id][blip2_question_index][label_index].append((match_type, 0.60))
                            elif label_verb == blip2_verb:
                                match_type = constants.BLIP2_VERB_LABEL_VERB
                                if label_index not in frame_id_predicted_label_indices_and_scores_dictionary_matching[frame_id][blip2_question_index].keys():
                                    frame_id_predicted_label_indices_and_scores_dictionary_matching[frame_id][blip2_question_index][label_index] = []
                                frame_id_predicted_label_indices_and_scores_dictionary_matching[frame_id][blip2_question_index][label_index].append((match_type, 0.40))
                            elif label_noun == blip2_noun:
                                match_type = constants.BLIP2_NOUN_LABEL_NOUN
                                if label_index not in frame_id_predicted_label_indices_and_scores_dictionary_matching[frame_id][blip2_question_index].keys():
                                    frame_id_predicted_label_indices_and_scores_dictionary_matching[frame_id][blip2_question_index][label_index] = []

                                frame_id_predicted_label_indices_and_scores_dictionary_matching[frame_id][blip2_question_index][label_index].append((match_type, 0.20))
    return (
        clip_id,
...
if __name__ == "__main__":
    ...
    for clip_id, frame_id_verb_noun_tool_pairs_mapping in tqdm(clip_id_frame_id_verb_noun_tool_pairs_mapping.items()):
        current_clip_id_frame_id_predicted_label_indices_and_scores_dictionary_matching[clip_id] = blip2_dictionary_matching_for_given_clip(
