...
class ConvTransformerBackbone(nn.Module):
    ...
    def forward(self, x, mask, use_xl=False):
        ...
        for idx in range(len(self.branch)):
            if self.use_xl:
                if idx == 0:
                    ...
                    x = x.permute(0, 2, 1)
                    x = self.xlnet(
                        inputs_embeds=x, attention_mask=mask.squeeze(1).long()
                    )[0]
                    x = x.permute(0, 2, 1)
                    # =================== xlnet ==========================#
            else:
                if idx == 0:
                    x, mask = self.stem[idx](x, mask)
            x, mask = self.branch[idx](x, mask)  # [b,c,t] [b,1,t]
        ...
        return (
            out_feats,
            out_masks,
        )  # 6 layer of different scale (continuously downsample)
