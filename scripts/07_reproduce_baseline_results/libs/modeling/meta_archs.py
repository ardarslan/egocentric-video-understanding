...
class PtTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.mu_reg_left = nn.Parameter(
            -torch.ones(self.num_classes, 1) * 0.5, requires_grad=True
        )
        self.sigma_reg_left = nn.Parameter(
            torch.ones(self.num_classes, 1), requires_grad=True
        )
        self.mu_reg_right = nn.Parameter(
            torch.ones(self.num_classes, 1) * 0.5, requires_grad=True
        )
        self.sigma_reg_right = nn.Parameter(
            torch.ones(self.num_classes, 1), requires_grad=True
        )
    ...
    def forward(self, video_list, ensemble=False, hidden_state=False, is_training=True):
        ...
        batched_inputs, batched_masks, segmentation_labels = self.preprocessing(
            video_list, is_training
        )  # pad to fixed length

        # forward the network (backbone -> neck -> heads)
        feats, masks = self.backbone(
            batched_inputs, batched_masks
        )  # List [b,c,ti].  List [b,1,ti]
        ...
        points = self.point_generator(
            fpn_feats
        )  # 4 means [t, reg_range_left, reg_range_right, fpn_stride]
        ...
        if is_training:
            ...
            assert (
                video_list[0]["segments"] is not None
            ), "GT action labels does not exist"
            assert (
                video_list[0]["labels"] is not None
            ), "GT action labels does not exist"
            gt_segments = [
                x["segments"].to(self.device)
                for x in video_list
                if len(x["labels"]) > 0
            ]
            gt_labels = [
                x["labels"].to(self.device) for x in video_list if len(x["labels"]) > 0
            ]
            ...
            (
                ...
                normal_probs_cls,
                normal_probs_reg,
            ) = self.label_points(points, gt_segments, gt_labels)
            ...
            losses = self.losses(
                ...
                out_offsets,  #
                gt_cls_labels,
                gt_offsets,  # 相当于fpn那么多层，每个点有一个gt边界和gt类，同时每一个点有预测的gt边界和gt类，算loss
                ...
                out_start=out_lb_logits,
                out_end=out_rb_logits,
            )
            # ============= other loss
    ...
    def preprocessing(self, video_list, is_training=True, padding_val=0.0):
        ...
        if is_training:
            ...
            ), "Input length must be smaller than max_seq_len during training"
            ...
            segmentation_labels_shape = [
                len(segmentation_labels),
                max_len,
                segmentation_labels[0].shape[-1],
            ]
            batched_segmentation_labels = segmentation_labels[0].new_full(
                segmentation_labels_shape, 0.0
            )
            for labels, pad_labels in zip(
                segmentation_labels, batched_segmentation_labels
            ):
            ...
            batched_inputs = F.pad(feats[0], padding_size, value=padding_val).unsqueeze(
                0
            )
    ...
    def label_points(self, points, gt_segments, gt_labels, for_seg=False):
        ...
        for gt_segment, gt_label in zip(gt_segments, gt_labels):
            ...
            if not for_seg:
                (
                    cls_targets,
                    reg_targets,
                    (normal_prob_cls, normal_prob_reg_left, normal_prob_reg_right),
                ) = self.label_points_single_video(concat_points, gt_segment, gt_label)
            else:
        ...
        return (
            ...
            normal_probs_cls,
            normal_probs_reg,
        )  # [378,1] [378,2] for each item in batch
    ...
    def label_points_single_video(self, concat_points, gt_segment, gt_label):
        ...
        num_pts = concat_points.shape[
            0
        ]  # all fpn level points   2046 = 1024+ 512....  [numpts,4]
        ...
        lens = lens[None, :].repeat(
            num_pts, 1
        )  # 扩充到num_pts，num_pts=2046, [numsegs, numpts]
        ...
        normal_prob_cls = normal_distribution(
            dist2center / (concat_points[:, 3, None] * lens),
            self.mu[gt_label].permute(1, 0),
            self.sigma[gt_label].permute(1, 0),
        )  # [num_pts, num_segs]
        normal_prob_reg_left = normal_distribution(
            dist2center / (concat_points[:, 3, None] * lens),
            self.mu_reg_left[gt_label].permute(1, 0),
            self.sigma_reg_left[gt_label].permute(1, 0),
        )  # [num_pts, num_segs]
        normal_prob_reg_right = normal_distribution(
            dist2center / (concat_points[:, 3, None] * lens),
            self.mu_reg_right[gt_label].permute(1, 0),
            self.sigma_reg_right[gt_label].permute(1, 0),
        )  # [num_pts, num_segs]
        reg_targets = torch.stack(
            (left, right), dim=-1
        )  # [numpts, num_segs, 2]， 每个当前点距离gt left和right的距离

        if self.train_center_sample == "radius":
            # center of all segments F T x N
            center_pts = 0.5 * (
                gt_segs[:, :, 0] + gt_segs[:, :, 1]
            )  # gt seg center [numpts, 1]
            t_mins = (
            ...
            )  # 判断action是否在这个proposal里 [num_pts, num_segs]
        else:
            # inside an gt action
        ...
        inside_regress_range = (
            torch.logical_and(  # [num_pts, num_segs] 判定符合regress range的点
        ...
        normal_prob_reg_left = normal_prob_reg_left[
            range(num_pts), min_len_inds
        ]  # [numpts]
        normal_prob_reg_right = normal_prob_reg_right[
            range(num_pts), min_len_inds
        ]  # [numpts]
    ...
    def losses(
        ...
        if label_list is not None:
            ...
            max_per_class_score = torch.max(max_per_class_score.softmax(-1), dim=1)[
                0
            ]  # [b, c]
            ...
            al_loss = (
                -involved_identify * max_per_class_score.log()
                - (1 - involved_identify) * (1 - max_per_class_score).log()
            )
            al_loss = al_loss.sum()
        ...
        if num_pos == 0:
            ...
            reg_loss = ctr_diou_loss_1d(pred_offsets, gt_offsets, reduction="None")
            normal_probs_reg_left[~pos_mask] = 1.0
            normal_probs_reg_right[~pos_mask] = 1.0
            reg_loss *= (
                normal_probs_reg_left[pos_mask] + normal_probs_reg_right[pos_mask]
            ) / 2.0
    ...
    def label_points_single_video_for_seg(self, concat_points, gt_segment, gt_label):
        ...
        num_pts = concat_points.shape[
            0
        ]  # 192 + 96 + 49 + 24 + 12 + 6   2046 = 1024+ 512....  [numpts,4]
        num_gts = gt_segment.shape[0]  # 1
    ...
    def get_segment_level_contra_loss(self, video_features, segments, labels):
        ...
        for i in range(video_features.shape[0]):
            segment = segments[i]
            theta_lens = ((segment[:, 1] - segment[:, 0]) * self.length_theta).clamp(
                min=1.0
            )  # [#]
            ...
            center_rois = self._to_roi_align_format(
                center_segs.unsqueeze(0), video_features[i].shape[-1]
            )
            center_roi_feature = self.roi_extractor(
                video_features[i].unsqueeze(0), center_rois
            ).mean(
                -1
            )  # [#, d, 16]
            ...
            left_rois = self._to_roi_align_format(
                left_segs.unsqueeze(0), video_features[i].shape[-1]
            )
            left_roi_feature = self.roi_extractor(
                video_features[i].unsqueeze(0), left_rois
            ).mean(-1)
            # right
            right_segs = copy.deepcopy(segment)
            right_segs[:, 0] = right_segs[:, 1] - theta_lens
            right_rois = self._to_roi_align_format(
                right_segs.unsqueeze(0), video_features[i].shape[-1]
            )
            right_roi_feature = self.roi_extractor(
                video_features[i].unsqueeze(0), right_rois
            ).mean(-1)
            ...
            left_out_rois = self._to_roi_align_format(
                left_out_segs.unsqueeze(0), video_features[i].shape[-1]
            )
            left_out_roi_feature = self.roi_extractor(
                video_features[i].unsqueeze(0), left_out_rois
            ).mean(-1)
            # right
            right_out_segs = copy.deepcopy(segment)
            right_out_segs[:, 0] = (right_out_segs[:, 1] + self.gap).clamp(
                max=video_features[i].shape[-1]
            )
            right_out_segs[:, 1] = (right_out_segs[:, 0] + theta_lens).clamp(
                max=video_features[i].shape[-1]
            )
            right_out_rois = self._to_roi_align_format(
                right_out_segs.unsqueeze(0), video_features[i].shape[-1]
            )
            right_out_roi_feature = self.roi_extractor(
                video_features[i].unsqueeze(0), right_out_rois
            ).mean(-1)
        ...
        pn_mask = (
            F.interpolate(pn_mask, size=(num * 3, num * 3), mode="nearest")
            .squeeze(0)
            .squeeze(0)
        )  # [3#,3#]
        # pn_mask[torch.arange(-1,num*3,3), :] = 0.0
        pn_mask[:, torch.arange(-1, num * 3, 3)] = 0.0
        ignore_mask = torch.ones((num * 3, num * 3), dtype=torch.float).to(
            labels.device
        )
...
class FeatureAlign(nn.Module):
    def __init__(self, d_in, d_out, num_layers=2, kernel_size=3, deformable_groups=1):
        ...
        for i in range(num_layers):
            ...
            #                             1, bias=False)
            conv_adapt = PackedDeformConv1d(
                d_in, d_out, kernel_size=kernel_size, padding="same"
            )
    ...
    def forward(self, fpn_feats, fpn_masks, out_offsets):
        ...
        for l, (cur_feat, cur_mask) in enumerate(zip(fpn_feats, fpn_masks)):
            ...
            for i in range(len(self.convs_adapt)):
                ...
                cur_feat, cur_offset = self.convs_adapt[i](
                    cur_feat, offsets=cur_offset, with_offsets=True
                )
