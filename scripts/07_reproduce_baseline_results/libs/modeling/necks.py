...
class FPN1D(nn.Module):
    ...
    def __init__(
        ...
        self.ac_conv = ACConv(
            d_in=in_channels[0], d_out=in_channels[0]
        )  # assume all dim same
...
class FPNIdentity(nn.Module):
    ...
    def forward(self, inputs, fpn_masks):
        ...
        if self.use_us_fpn:
            ...
            last_lateral = self.lateral_linears[-1](
                inputs[-1].permute(0, 2, 1)
            ).permute(
                0, 2, 1
            )  # [b,c,t]
            last_lateral = resize(
                last_lateral, size=(out_size), mode="linear", align_corners=False
            )
            ...
            for i, feat in enumerate(inputs):
                ...
                lateral = (
                    self.mx_linears[i](last_lateral.permute(0, 2, 1)).permute(0, 2, 1)
                    + tmp
                )
                laterals.append(lateral)
            concat_lateral = torch.cat(laterals, dim=1)
            concat_lateral = self.post_linear(concat_lateral.permute(0, 2, 1)).permute(
                0, 2, 1
            )

            first_mask = fpn_masks[self.start_level]
            first_mask_out = F.interpolate(
                first_mask.to(x.dtype), size=out_size, mode="nearest"
            )
            first_mask_out = first_mask_out.bool()
