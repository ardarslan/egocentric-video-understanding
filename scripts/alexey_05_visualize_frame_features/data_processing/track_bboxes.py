...
def process(process_idx, data):
    ...
    if "hand_bbox" in data_types:
        data_types = [
            *[d for d in data_types if d != "hand_bbox"],
            "hand_bbox_left",
            "hand_bbox_right",
        ]
    ...
    if "hos_hands" in data_types:
        data_types = [
            *[d for d in data_types if d != "hos_hands"],
    ...
    if "hos" in data_types:
        data_types = [
            *[d for d in data_types if d != "hos"],
            *[f"hos_left_hand_{v}" for v in available_hos_versions],
            *[f"hos_right_hand_{v}" for v in available_hos_versions],
    ...
    if "object_bbox" in data_types:
        data_types = [
            *[d for d in data_types if d != "object_bbox"],
    ...
    def init_dict():
        return {
            "session_timestamp": int(timestamp),
            "args": args,
            "video_id": video_id,
            ...
            "frame_original_box_idxs_to_tracks": {
                data_type: {} for data_type in data_types
            },
            "frame_original_box_idxs_to_tracks_segmented": {
                "gt": {data_type: {} for data_type in data_types}
            },
        }
    ...
    for video_idx, video_id in tqdm(enumerate(video_ids)):
        data_dict = {**init_dict(), "exception": None}
        ...
        try:
            trackers = {
                data_type: MultiObjectTracker(dt=1.0 / reader.fps)
            ...
            for frame_idx in range_obj:
                ...
                for data_type in data_types:
                    ...
                    data_dict["frame_original_box_idxs_to_tracks"][data_type][
                        frame_id
                    ] = {}
                    data_dict["frame_original_box_idxs_to_tracks_segmented"]["gt"][
                        data_type
                    ][frame_id] = {}

                    if "hand_bbox" in data_type:
                        path_func = CHANNEL_FRAME_PATH_FUNCTS[
                            "hand_bbox"
                            if "hand_bbox" in data_type
                            else "hos"
                            if data_type.startswith("hos_")
                            else data_type
                        ]
                    ...
                    if not any(
                        (in_path.lower().endswith(ext) for ext in [".pkl", ".zip"])
                    ):
                        continue
                    ...
                    if data_type.startswith("segmentation_mask"):
                        ...
                        for frame_box_idx, frame_box in enumerate(external_data):
                            ...
                            for mask_idx, mask in enumerate(masks):
                                ...
                                mask_bbox_global = [
                                    outer_bbox[0] + mask_bbox[0],
                                    outer_bbox[1] + mask_bbox[1],
                                    outer_bbox[0] + mask_bbox[2],
                                    outer_bbox[1] + mask_bbox[3],
                                ]

                                frame_boxes_idxs.append(
                                    [mask_bbox_global, (frame_box_idx, mask_idx)]
                                )
                                frame_data_type_detections.append(
                                    Detection(box=get_motpy_box(mask_bbox_global))
                                )
                        ...
                        if (
                            not isinstance(external_data, list)
                            or len(external_data) < 2
                        ):
                            continue
                        ...
                        if not hasattr(external_data["instances"], "pred_handsides"):
                            print(
                                f'not hasattr(external_data["instances"], "pred_handsides"): {in_path}'
                            )
                            continue

                        for cls, handside, mask, box_tensor in zip(
                            external_data["instances"].pred_classes,
                    ...
                    for detection_idx, track_id in enumerate(
                        trackers[data_type].detections_matched_ids
                    ):
                        ...
                        for candidate_box, original_box_idx in frame_boxes_idxs:
                            if (
                                np.sum(
                                    np.abs(
                                        np.array(actual_original_box)
                                        - np.array(candidate_box)
                                    )
                                )
                                < 1e-6
                            ):
                                ...
                                if (
                                    ...
                                    data_dict["frame_original_box_idxs_to_tracks"][
                                        data_type
                                    ][frame_id][original_box_idx] = [track_id]
                                    frame_dict["frame_original_box_idxs_to_tracks"][
                                        data_type
                                    ][original_box_idx] = [track_id]

                                    data_dict[
                                        "frame_original_box_idxs_to_tracks_segmented"
                                    ]["gt"][data_type][frame_id][original_box_idx] = [
                                        track_id + suffix
                                    ]
                                    frame_dict[
                                        "frame_original_box_idxs_to_tracks_segmented"
                                    ]["gt"][data_type][original_box_idx] = [
                                        track_id + suffix
                                    ]
                                else:
                                    data_dict["frame_original_box_idxs_to_tracks"][
                                        data_type
                                    ][frame_id][original_box_idx].append(track_id)
                                    frame_dict["frame_original_box_idxs_to_tracks"][
                                        data_type
                                    ][original_box_idx].append(track_id)

                                    data_dict[
                                        "frame_original_box_idxs_to_tracks_segmented"
                                    ]["gt"][data_type][frame_id][
                                        original_box_idx
                                    ].append(
                                        [track_id + suffix]
                                    )
                                    frame_dict[
                                        "frame_original_box_idxs_to_tracks_segmented"
                                    ]["gt"][data_type][original_box_idx].append(
                                        [track_id + suffix]
                                    )
                        ...
                        if (
                            track_id + suffix
                            not in data_dict["tracks_segmented"]["gt"][data_type]
                        ):
                            data_dict["tracks_segmented"]["gt"][data_type][
                                track_id + suffix
                            ] = {}

                        data_dict["tracks"][data_type][track_id][frame_id] = {
                            "box": track.box,
                            "score": track.score,
                            "original_box_idxs": original_box_idxs,
                        }
                        frame_dict["tracks"][data_type][track_id] = {
                            "box": track.box,
                            "score": track.score,
                            "original_box_idxs": original_box_idxs,
                        }
                        ...
                        frame_dict["tracks_segmented"]["gt"][data_type][
                            track_id + suffix
                        ] = {
                            "box": track.box,
                            "score": track.score,
                ...
                if num_processed_frames % 1000 == 0:
                    print(
                        f"Process {process_idx}: at {frame_id} ({num_processed_frames} frames , {video_idx}/{len(video_ids)} videos)"
                    )
                    save_pkl()
...
def main(arg_dict=None):
    ...
    parser.add_argument(
        "--output_dir", type=str, default=join(ROOT_PATH, "data", "EK_bbox_tracks")
    )
    ...
    parser.add_argument(
        ...
        action="append",
        type=str,
        default=["hos,hand_bbox,object_bbox,segmentation_mask"],
    )
