...
register_epick_instances(
    "epick_visor_2022_val_hos", {}, f"{version}/annotations/val.json", f"{version}/val"
)
    ...
    for video_idx, video_id in tqdm(enumerate(args.generator_videos)):
        ...
        if True:
            ...
            for frame_idx in range_obj:
                ...
                pkl_out_path = os.path.join(
                    out_dir, video_id, "pkls", frame_id + ".pkl"
                )
                ...
                if args.egohos_max_hand_intersection_ioa != 1.0:
                    ...
                    if isfile(egohos_path):
                        ...
                        if (
                            egohos_data.shape[0] != im.shape[0]
                            or egohos_data.shape[1] != im.shape[1]
                        ):
                            egohos_data_img = Image.fromarray(egohos_data)
                            egohos_data_img = egohos_data_img.resize(
                                (egohos_data.shape[1], egohos_data.shape[0]),
                                Image.NEAREST,
                            )
                        ...
                        if len(pred_class_list) > 0:
                            ...
                            new_instances.pred_handsides = torch.stack(
                                pred_handside_list
                            )
                            new_instances.pred_masks = torch.stack(
                                pred_mask_list
                            ).cuda()
                ...
                with zipfile.ZipFile(
                    ...
                    zip_file.writestr(
                        ...
                        pickle.dumps({**outputs, **image_size_data}),
                    )
