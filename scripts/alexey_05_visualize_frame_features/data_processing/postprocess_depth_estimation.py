...
def visualize_depth(depth, depth_min=None, depth_max=None):
    ...
    return ((cv2.applyColorMap(depth_scaled_uint8, color_map_magma) / 255) ** 2.2) * 255
...
def process(process_idx, data):
    ...
    for video_id, dir_paths in zip(video_ids, dir_paths_top):
        ...
        for dir_path in reversed(sorted(dir_paths)):
            ...
            original_start_frame_idx = next(
                (
                    int(spl[2:])
                    for spl in basename(dir_path).split("_")
                    if spl.startswith("OS")
                ),
                None,
            )
            ...
            aggregated_output_dirname = next(
                filter(lambda n: n.startswith("R"), os.listdir(dir_path)), None
            )
            if aggregated_output_dirname is not None and isdir(
                aggregated_output_dir_path := join(
                    dir_path,
                    aggregated_output_dirname,
                    "StD100.0_StR1.0_SmD0_SmR0.0",
                    "depth_e0000",
                    "e0000_filtered",
                    "depth",
                )
            ...
            for version, dir_to_scan in dirs_to_scan:
                for fn in sorted(os.listdir(dir_to_scan)):
                    ...
                    original_frame_delta_idx = next(
                        (
                            int(spl)
                            for spl in fn.replace(".", "_").split("_")
                            if spl.isnumeric()
                        ),
                        None,
                    )
                    if original_frame_delta_idx is not None:
                        original_frame_idx = (
                            original_start_frame_idx + original_frame_delta_idx
                        )
                        virtual_frame_idx = reader.get_virtual_frame_idx(
                            original_frame_idx
                        )
                        frame_id = fmt_frame(video_id, virtual_frame_idx)
                        video_output_path = join(
                            args.output_dir,
                            version + "_video_scale",
                            video_id,
                            f"{frame_id}.png",
                        )
                        section_output_path = join(
                            args.output_dir,
                            version + "_section_scale",
                            video_id,
                            f"{frame_id}.png",
                        )
                        ...
                        video_min_depth_full[version] = min(
                            video_min_depth_full[version], lower_full
                        )
                        video_max_depth_full[version] = max(
                            video_max_depth_full[version], upper_full
                        )
                        section_min_depth[version] = min(
                            section_min_depth[version], lower
                        )
                        section_max_depth[version] = max(
                            section_max_depth[version], upper
                        )
                        section_min_depth_full[version] = min(
                            section_min_depth_full[version], lower_full
                        )
                        section_max_depth_full[version] = max(
                            section_max_depth_full[version], upper_full
                        )

                        if not isfile(video_output_path) or not isfile(
                            section_output_path
                        ):
                            input_output_paths.append(
                                (
                                    version,
                                    input_path,
                                    video_output_path,
                ...
                scaling_dict[version][original_start_frame_idx] = {
                    "min": section_min_depth[version],
        ...
        for version in ["midas2", "cvd"]:
            ...
            summary_output_path = join(
                args.output_dir, version + "_video_scale", video_id, "_summary.pkl"
            )
            os.makedirs(dirname(summary_output_path), exist_ok=True)
            with open(summary_output_path, "wb") as f:
                pickle.dump(
                    {
                        "section_scaling": OrderedDict(
                            sorted(scaling_dict[version].items())
                        ),
                        "video_scaling": {
                            "min": video_min_depth[version],
                            "max": video_max_depth[version],
                            "min_full": video_min_depth_full[version],
                            "max_full": video_max_depth_full[version],
                        },
        ...
        for (
            ...
            video_output_path,
            section_output_path,
        ) in input_output_paths:
            input_data = load_raw_float32_image(input_path)

            color_img_video_np = visualize_depth(
                input_data, video_min_depth[version], video_max_depth[version]
            )
            color_img_video = Image.fromarray(
                color_img_video_np[:, :, ::-1].astype(np.uint8)
            )
            color_img_video_res = color_img_video.resize(
                (reader.video_width, reader.video_height), Image.BICUBIC
            )

            color_img_section_np = visualize_depth(
                input_data, section_min_depth[version], section_max_depth[version]
            )
            color_img_section = Image.fromarray(
                color_img_section_np[:, :, ::-1].astype(np.uint8)
            )
            color_img_section_res = color_img_section.resize(
                (reader.video_width, reader.video_height), Image.BICUBIC
            )
            ...
            print(
                f"Converted {input_path}  -->  {video_output_path} & {section_output_path}"
            )
...
def main(arg_dict=None):
    ...
    parser.add_argument(
        "--output_dir", type=str, default=DEPTH_ESTIMATION_POSTPROCESSING_DATA_DIR
    )
        ...
        with Pool(processes=args.num_jobs) as pool:
            keys_split = list(
                map(
                    ...
                    np.array_split(list(dirs_to_process.keys()), args.num_jobs),
                )
            )
            pool.starmap(
                process,
                enumerate(
                    zip(
                        [args] * args.num_job,
                        keys_split,
                        [[dirs_to_process[k] for k in ks] for ks in keys_split],
                    )
                ),
            )
