...
def run_hand_mocap(args, bbox_detector, hand_mocap, visualizer, generator_videos=None):
    ...
    for video_id in generator_videos:
        generator = get_action_recognition_frame_gen(["val"], videos=[video_id], max_width=1280, max_height=720,
                                                     action_frames_only=not args.full_video)  # , max_width=640, max_height=480

        if args.out_dir in [None, ""]:
            args.out_dir = CHANNEL_VIDEO_PATH_FUNCTS["hand_mesh"](video_id, args.version)
        ...
        while True:
            ...
            try:
                pkl_out_path = join(args.out_dir, "mocap", f"{frame_id}_prediction_result.pkl")  # join(args.out_dir, "mocap", f"{frame_id}_prediction_result.pkl")

                if os.path.isfile(pkl_out_path) or os.path.isfile(pkl_out_path + ".zip"):
                    continue
                ...
                if args.save_pred_pkl:
                    ...
                    with zipfile.ZipFile(pkl_out_path + ".zip", "w",
                        ...
                        with open(pkl_out_path, "rb") as pkl_file:
                            zip_file.writestr(os.path.basename(pkl_out_path),
                                              pickle.dumps({**pickle.load(pkl_file),
