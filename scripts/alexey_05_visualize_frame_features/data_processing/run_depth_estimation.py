...
def main(arg_dict=None):
    ...
    try:
        from data_processing.extract_actionwise_videos import (
            main as extract_actionwise_videos,
        )

        video_ids_and_paths = extract_actionwise_videos(
            {"generator_videos": args.generator_videos}
        )
    ...
    for video_id, path in video_ids_and_paths:
        ...
        os.system(
            (
                f"python main.py --video_file {path} --path {output_path} --save_intermediate_depth_streams_freq 1 "
