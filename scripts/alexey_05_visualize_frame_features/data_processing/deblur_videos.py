...
def main(arg_dict=None):
    ...
    gen = get_action_recognition_frame_gen(subsets=["val"],
                                           add_deltas=([-(i+1) for i in range(args.before_frames)]
    ...
    with torch.no_grad():
        ...
        net.load_state_dict(torch.load(join(SHIFTNET_PATH, "pretrained_models", "net_gopro_deblur.pth"))["params"])
        net.half()
        ...
        for frame_data in tqdm(gen):
            ...
            output_array = output.float().clamp(0,1.0).permute(1,2,0).cpu().numpy() * 255
