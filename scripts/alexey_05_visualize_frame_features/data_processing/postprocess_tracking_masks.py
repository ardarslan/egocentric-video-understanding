...
def process(process_idx, data):
    font = ImageFont.truetype(
        "/mnt/scratch/agavryushin/Thesis/webserver/fonts/DejaVuSans.ttf", 20
    )
            ...
            for hos_version in args.hos_version:
                for min_length in args.min_length:
                    for track_type in args.track_type:
                        tracking_mask_path = CHANNEL_VIDEO_PATH_FUNCTS["tracking_mask"](
                        ...
                        print(
                            f"Process {process_idx}: {image_version=}, {hos_version=}, {min_length=}, {track_type=}"
                        )

                        os.makedirs(
                            join(ROOT_PATH, "data", "contours_vis", video_id),
                            exist_ok=True,
                        )

                        tracks = os.listdir(tracking_mask_path)
                        for track_idx, dn in enumerate(tracks):
                            if args.track_filter is not None and not any(
                                [dn.startswith(s) for s in args.track_filter]
                            ):
                                print(
                                    f'Ignoring track "{dn}" by filter',
                                    args.track_filter,
                                )
                                continue
                            ...
                            if isdir(dir_path):
                                ...
                                track_avg_quantiles = {
                                    q: [] for q in np.arange(0, 1.01, 0.01)
                                }
                                track_cd_avgs = []
                                track_cd_stds = []
                                ...
                                for frame_idx in (progress_bar := tqdm(range_obj)):
                                    ...
                                    if isfile(path):
                                        try:
                                            ...
                                            image_diag_size = math.sqrt(
                                                masks.shape[0] ** 2.0
                                                + masks.shape[1] ** 2.0
                                            )
                                            line_length_px = round(
                                                args.relative_segment_size
                                                * image_diag_size
                                            )
                                            (
                                                *_,
                                            ...
                                            if num_contours > 0:
                                                if dn not in seen_tracks:
                                                    ...
                                                    for seen_track_id in seen_tracks:
                                                        ...
                                                        tracking_mask_path_2 = join(
                                                            tracking_mask_dir_2,
                                                            f"{frame_id}__{dn}.pkl.zip",
                                                        )
                                                        if isfile(tracking_mask_path_2):
                                                            data_2 = read_pkl(
                                                                tracking_mask_path_2
                                                            )
                                                            track_summary_dict[
                                                                "initial_frame_intersections"
                                                            ][
                                                ...
                                                for (
                                                    ...
                                                    while pos < len(contour):
                                                        x = contour[pos][0][0]
                                                        y = contour[pos][0][1]
                                                        this_frame_all_contour_points.append(
                                                            [x, y]
                                                        )
                                                        this_contour_rel_points.append(
                                                            [
                                                                x / track_mask_width,
                                                                y / track_mask_height,
                                                            ]
                                                    ...
                                                    this_frame_all_contours_images.append(
                                                        contour_img
                                                    )
                                                ...
                                                final_tortuosity = sum(
                                                    [t[0] * t[1] for t in tortuosities]
                                                ) / max(
                                                    1, sum([t[1] for t in tortuosities])
                                                )

                                                track_final_tortuosities.append(
                                                    final_tortuosity
                                                )
                                                ...
                                                output_dict.update(
                                                    {
                                                        "tortuosity": final_tortuosity,
                                                        "tortuosity_ema": pd.Series(
                                                            track_final_tortuosities[
                                                                -ewm_span_abs:
                                                            ]
                                                        )
                                                        .ewm(span=ewm_span_abs)
                                                        .mean()
                                                        .iloc[-1],
                                                ...
                                                if isfile(segmentation_mask_path):
                                                    segmentation_mask_data = read_pkl(
                                                        segmentation_mask_path
                                                    )
                                                    ...
                                                    for (
                                                        top_level_box
                                                    ) in segmentation_mask_data:
                                                        box = top_level_box["box"]
                                                        cls = top_level_box["cls"]
                                                        seg_masks = top_level_box[
                                                            "masks"
                                                        ]
                                                        for mask in seg_masks:
                                                            ...
                                                            for (
                                                                ...
                                                                for entry in contour:
                                                                    x = entry[0][0]
                                                                    y = entry[0][1]
                                                                    this_frame_all_mask_contours.append(
                                                                        [x, y]
                                                                    )
                                                    ...
                                                    output_dict["cd_avg"] = cd / len(
                                                        this_frame_all_contour_points
                                                    )
                                                    output_dict["cd_std"] = np.std(
                                                        min_x_to_y
                                                    )
                                                    track_cd_avgs.append(
                                                        output_dict["cd_avg"]
                                                    )
                                                    track_cd_stds.append(
                                                        output_dict["cd_std"]
                                                    )
                                                    ...
                                                    for q_idx, q in enumerate(
                                                        ...
                                                        track_avg_quantiles[q].append(
                                                            output_dict[
                                                                "cd_avg_quantiles"
                                                            ][q_idx]
                                                        )

                                                    output_dict["cd_avgs_ema"] = (
                                                        pd.Series(
                                                            track_cd_avgs[
                                                                -ewm_span_abs:
                                                            ]
                                                    ...
                                                    output_dict[
                                                        "cd_avg_quantiles_ema"
                                                    ] = np.array(
                                                        [
                                                            pd.Series(
                                                                track_avg_quantiles[q][
                                                                    -ewm_span_abs:
                                                                ]
                                                            )
                                                            .ewm(span=ewm_span_abs)
                                                    ...
                                                    progress_bar.set_description(
                                                        f'{frame_id=} {dn=} (#{track_idx+1}/{len(tracks)}) {output_dict["cd_avg"]=} {output_dict["cd_std"]=}'
                                                    )
                                                    # get CD
                                                ...
                                                object_bbox_path = object_bbox_path_fn(
                                                    video_id,
                                                    frame_idx,
                                                    frame_id,
                                                ...
                                                if isfile(object_bbox_path):
                                                    object_bbox_data = read_pkl(
                                                        object_bbox_path
                                                    )

                                                    for major_idx in range(
                                                        len(object_bbox_data["boxes"])
                                                    ):
                                                        for box, cls, score in list(
                                                            zip(
                                                                object_bbox_data[
                                                                    "boxes"
                                                                ][major_idx],
                                                            ...
                                                            if check_boxes_intersect(
                                                                box_rel, bbox_rel
                                                            ):
                                                                track_summary_dict[
                                                                    "track_object_intersection_count"
                                                                ] += 1
                                                                bbox_intersection = get_bbox_intersection(
                                                                    box_rel, bbox_rel
                                                                )
                                                                track_summary_dict[
                                                                    "track_object_intersection_counts"
                                                                ...
                                                                bbox_intersection_area = get_bbox_area(
                                                                    bbox_intersection
                                                                )
                                                                bbox_union_area = (
                                                                    get_bbox_area(
                                                ...
                                                hos_hands_path = hos_hands_path_fn(
                                                    video_id,
                                                    frame_idx,
                                                    frame_id,
                                                    args.hos_version_hands,
                                                )

                                                if isfile(hos_hands_path):
                                                    hos_hands_data = read_pkl(
                                                        hos_hands_path
                                                    )
                                                    if (
                                                        args.hos_version_hands
                                                ...
                                                hos_object_path = hos_object_path_fn(
                                                    video_id,
                                                    frame_idx,
                                                    frame_id,
                                                    args.hos_version_object,
                                                )

                                                if isfile(hos_object_path):
                                                    hos_object_data = read_pkl(
                                                        hos_object_path
                                                    )
                                                    if (
                                                        args.hos_version_object
                                                ...
                                                if hand_mask is not None:
                                                    hands_iou = calculate_iou(
                                                        masks, hand_mask
                                                    )
                                                    # smaller denominator -> larger IoA
                                                    hands_ioa = calculate_ioa(
                                                        masks,
                                                        hand_mask,
                                                        denominator="min",
                                                    )
                                                    output_dict[
                                                        "track_hos_hands_intersection_iou"
                                                    ] = hands_iou
                                                    output_dict[
                                                        "track_hos_hands_intersection_ioa"
                                                    ] = hands_ioa
                                                    track_summary_dict[
                                                        "track_hos_hands_intersection_ious"
                                                    ][frame_id] = hands_iou
                                                    ...
                                                    track_hos_hands_frame_idxs.append(
                                                        frame_idx
                                                    )
                                                    track_hos_hands_ioas.append(
                                                        hands_ioa
                                                    )
                                                    track_hos_hands_ious.append(
                                                        hands_iou
                                                    )
                                                ...
                                                if len(track_hos_hands_frame_idxs) > 0:
                                                    ...
                                                    output_dict[
                                                        "track_hos_hands_intersection_iou_ema"
                                                    ] = hands_iou_ema
                                                    output_dict[
                                                        "track_hos_hands_intersection_ioa_ema"
                                                    ] = hands_ioa_ema

                                                if object_mask is not None:
                                                    object_iou = calculate_iou(
                                                        masks, object_mask
                                                    )
                                                    # larger denominator -> smaller IoA
                                                    object_ioa = calculate_ioa(
                                                        masks,
                                                        object_mask,
                                                        denominator="min",
                                                    )
                                                    ...
                                                    track_hos_object_frame_idxs.append(
                                                        frame_idx
                                                    )
                                                    track_hos_object_ioas.append(
                                                        object_ioa
                                                    )
                                                    track_hos_object_ious.append(
                                                        object_iou
                                                    )
                                                ...
                                                if len(track_hos_object_frame_idxs) > 0:
                                                    ...
                                                    output_dict[
                                                        "track_hos_object_intersection_iou_ema"
                                                    ] = object_iou_ema
                                                    output_dict[
                                                        "track_hos_object_intersection_ioa_ema"
                                                    ] = object_ioa_ema
                                            ...
                                            if masks.max() > 0:
                                                ...
                                                filter_passed = (
                                                    tortuosity_ema
                                                    <= args.tracking_mask_merging_max_tortuosity
                                                    and cd_90
                                                    <= args.tracking_mask_merging_max_cd_q90
                                                    and (
                                                        hand_ioa
                                                        if not np.isnan(hand_ioa)
                                                        else 0.0
                                                    )
                                                    <= args.tracking_mask_merging_max_hand_ioa
                                                )
                                                if filter_passed:
                                                    output_dict[
                                                        "intersection_check_performed"
                                                    ] = True
                                                    ...
                                                    for (
                                                        ...
                                                        if (
                                                            ...
                                                            and not any(
                                                                [
                                                                    other_track_id.startswith(
                                                                        s
                                                                    )
                                                                    for s in args.track_filter
                                                                ]
                                                            )
                                                        ):
                                                            continue
                                                        ...
                                                        other_track_mask_path = join(
                                                            other_tracking_mask_dir,
                                                            other_track_id,
                                                            f"{frame_id}__{other_track_id}.pkl.zip",
                                                        )
                                                        # load other track's frame info to check filter
                                                        other_postprocessing_data_path = join(
                                                            CHANNEL_FRAME_PATH_FUNCTS[
                                                                "tracking_mask_postprocessing"
                                                            ](
                                                                video_id,
                                                            ...
                                                            other_track_id,
                                                            f"{frame_id}__{other_track_id}.pkl",
                                                        )
                                                        if isfile(
                                                            other_track_mask_path
                                                        ) and isfile(
                                                            other_postprocessing_data_path
                                                        ):
                                                            other_postprocessing_data = read_pkl(
                                                                other_postprocessing_data_path
                                                            )
                                                            other_tortuosity_ema = (
                                                                other_postprocessing_data[
                                                                    "tortuosity_ema"
                                                                ]
                                                                if "tortuosity_ema"
                                                                in other_postprocessing_data
                                                                else np.nan
                                                            )
                                                            other_cd_90 = (
                                                                float(
                                                            ...
                                                            other_hand_ioa = (
                                                                ...
                                                                if other_postprocessing_data.get(
                                                                    "track_hos_hands_intersection_ioa_ema"
                                                                )
                                                                is not None
                                                                else np.nan
                                                            ...
                                                            if other_filter_passed:
                                                                other_track_mask_data = read_pkl(
                                                                    other_track_mask_path
                                                                )
                                                                other_mask_img = other_track_mask_data[
                                                                    "masks"
                                                                ]
                                                                if (
                                                                    other_mask_img.max()
                                                                    ...
                                                                    other_track_mask_bbox = bbox_from_mask(
                                                                        other_mask_img
                                                                    )
                                                                    other_track_mask_bbox_rel = scale_box(
                                                                        other_track_mask_bbox,
                                                                        other_track_mask_data.get(
                                                                            "track_mask_width",
                                                                            track_mask_width,
                                                                        ),
                                                                        other_track_mask_data.get(
                                                                    ...
                                                                    if check_boxes_intersect(
                                                                        track_mask_bbox_rel,
                                                                        other_track_mask_bbox_rel,
                                                                    ):
                                                                        (
                                                                            *_,
                                                                            other_mask_contours,
                                                                            other_mask_hierarchy_top,
                                                                        ) = cv2.findContours(
                                                                            other_mask_img,
                                                                            cv2.RETR_EXTERNAL,
                                                                            cv2.CHAIN_APPROX_NONE,
                                                                        ...
                                                                        if (
                                                                            ...
                                                                            track_summary_dict[
                                                                                "track_track_intersection_own_areas"
                                                                            ][
                                                                                other_track_id
                                                                            ] = {}
                                                                        ...
                                                                        if (
                                                                            ...
                                                                            this_frame_mask_image_res = Image.fromarray(
                                                                                masks.astype(
                                                                                    np.uint8
                                                                                )
                                                                            )
                                                                            this_frame_mask_image_res = this_frame_mask_image_res.resize(
                                                                                (
                                                                                    other_mask_img.shape[
                                                                                        1
                                                                                    ],
                                                                            ...
                                                                            this_frame_mask_image_res = np.array(
                                                                        ...
                                                                        intersection_img = np.logical_and(
                                                                            this_frame_mask_image_res,
                                                                            other_mask_img,
                                                                        )
                                                                        union_img = np.logical_or(
                                                                            this_frame_mask_image_res,
                                                                            other_mask_img,
                                                                        )
                                                                        mask_iou = intersection_img.sum() / max(
                                                                            1,
                                                                            union_img.sum(),
                                                                        )
                                                                        mask_ioa = intersection_img.sum() / max(
                                                                            1,
                                                                            this_frame_mask_image_res.sum(),
                                                                        )
                                                                        track_summary_dict[
                                                                            "track_track_intersection_ious"
                                                                        ][
                                                                        ...
                                                                        if (
                                                                            mask_iou
                                                                            >= args.tracking_mask_merging_overlap_frame_iou_fraction
                                                                        ):
                                                                            track_summary_dict[
                                                                                "track_track_intersection_ious_filtered"
                                                                            ][
                                            ...
                                            with open(
                                                join(out_dir, f"{frame_id}__{dn}.pkl"),
                                                "wb",
                                            ) as f:
                                ...
                                track_summary_dict["track_cd_avgs_avg"] = np.mean(
                                    track_cd_avgs
                                )
                                track_summary_dict["track_cd_avgs_std"] = np.std(
                                    track_cd_avgs
                                )
                                track_summary_dict["track_cd_stds_avg"] = np.mean(
                                    track_cd_avgs
                                )
                                track_summary_dict["track_cd_stds_std"] = np.std(
                                    track_cd_avgs
                                )

                                track_summary_dict[
                                    "track_mask_bbox_bottoms_avg"
                                ] = np.mean(track_mask_bbox_bottoms)
                                track_summary_dict[
                                    "track_mask_bbox_bottoms_std"
                                ] = np.std(track_mask_bbox_bottoms)
                                track_summary_dict[
                                    "track_mask_bbox_bottoms_quantiles"
                                ] = np.quantile(
                                    track_mask_bbox_bottoms, np.arange(0.0, 1.01, 0.01)
                                )

                                track_mask_bbox_bottoms_rel = (
                                    np.array(track_mask_bbox_bottoms)
                                    / track_mask_height
                                )
                                track_summary_dict[
                                    "track_mask_bbox_bottoms_avg_rel"
                                ] = np.mean(track_mask_bbox_bottoms_rel)
                                track_summary_dict[
                                    "track_mask_bbox_bottoms_std_rel"
                                ] = np.std(track_mask_bbox_bottoms_rel)
                                track_summary_dict[
                                    "track_mask_bbox_bottoms_quantiles_rel"
                                ] = np.quantile(
                                    track_mask_bbox_bottoms_rel,
                                    np.arange(0.0, 1.01, 0.01),
                                )
                                ...
                                for (
                                    intersection_track_id,
                                    intersection_data,
                                ) in track_summary_dict[
                                    "track_track_intersection_ious_filtered"
                                ].items():
                                    ...
                                    other_tracking_mask_dir = CHANNEL_VIDEO_PATH_FUNCTS[
                                        "tracking_mask_postprocessing"
                                    ](
                                        video_id,
                                        image_version,
                                    ...
                                    other_track_summary_path = join(
                                        other_tracking_mask_dir,
                                        intersection_track_id,
                                        f"_summary.pkl",
                                    )
                                    if isfile(other_track_summary_path):
                                        other_track_summary_data = read_pkl(
                                            other_track_summary_path
                                        )

                                        num_intersections = len(intersection_data)
                                        this_track_intersection_fraction = num_intersections / max(
                                            1,
                                            len(
                                                track_summary_dict[
                                                    "track_mask_appearances_filter2_passed"
                                        ...
                                        if (
                                            this_track_intersection_fraction
                                            >= args.tracking_mask_merging_overlap_frame_count_fraction
                                        ):
                                            track_summary_dict[
                                                "merge_track_into"
                                            ].append(intersection_track_id)
                                            print(
                                                f">>> MERGING: Will merge {dn} into {intersection_track_id}"
                                            )

                                        other_track_intersection_fraction = num_intersections / max(
                                            1,
                                            len(
                                                other_track_summary_data[
                                                    "track_mask_appearances_filter2_passed"
                                                ]
                                            ),
                                        )
                                        if (
                                            other_track_intersection_fraction
                                            >= args.tracking_mask_merging_overlap_frame_count_fraction
                                        ):
                                            other_track_summary_data[
                                                "merge_track_into"
                                            ...
                                            print(
                                                f">>> MERGING: Will merge {intersection_track_id} into {dn} (retroactively)"
                                            )
                                            with open(
                                                other_track_summary_path, "wb"
                                            ) as f:
                                                pickle.dump(other_track_summary_data, f)

                                summary_out_dir = join(
                                    CHANNEL_FRAME_PATH_FUNCTS[
                                        "tracking_mask_postprocessing"
                                    ](
                                        video_id,
                                ...
                                with open(
                                    join(summary_out_dir, "_summary.pkl"), "wb"
                                ) as f:
...
def main(arg_dict=None):
    ...
    parser.add_argument(
        "--object_bbox_version", type=str, default=DEFAULT_OBJECT_BBOX_VERSION
    )
    ...
    if args.image_version is None:
        ...
        args.image_version = [
            s.strip() for v in args.image_version for s in v.split(",")
        ]
    ...
    if args.num_jobs == 1:
        ...
        with Pool(processes=args.num_jobs) as pool:
            ...
            pool.starmap(
                ...
                enumerate(
                    zip(
                        [args] * args.num_jobs, [timestamp] * args.num_jobs, paths_split
                    )
                ),
            )
