...
def main(arg_dict=None):
    ...
    TWOHANDS_TO_CB_CONFIG_PATH = (
        "./work_dirs/twohands_to_cb_ccda/twohands_to_cb_ccda.py"
    )
    TWOHANDS_TO_CB_CHECKPOINT_PATH = (
        "./work_dirs/twohands_to_cb_ccda/best_mIoU_iter_76000.pth"
    )

    TWOHANDS_CB_TO_OBJ_CONFIG_PATH = (
        "./work_dirs/twohands_cb_to_obj1_ccda/twohands_cb_to_obj1_ccda.py"
    )
    TWOHANDS_CB_TO_OBJ_CHECKPOINT_PATH = (
        "./work_dirs/twohands_cb_to_obj1_ccda/best_mIoU_iter_34000.pth"
    )
    ...
    twohands_model = init_segmentor(
        TWOHANDS_CONFIG_PATH, TWOHANDS_CHECKPOINT_PATH, device=args.device
    )
        ...
        try:
            ...
            for frame_idx in tqdm(range_obj):
                ...
                if isfile(pkl_out_path + ".zip"):
                    ...
                    img = Image.fromarray(
                        read_pkl(pkl_out_path + ".zip").astype(np.uint8)
                    )
                    img.save(seg_result_png_path)
                    twohands_png_paths_videos[video_id].append(
                        (frame_idx, frame_id, seg_result_png_path)
                    )
                    continue
                else:
                    try:
                ...
                with zipfile.ZipFile(
                    f"{pkl_out_path}.zip", "w", zipfile.ZIP_DEFLATED, False
                ) as zip_file:
                    zip_file.writestr(
                        basename(pkl_out_path), pickle.dumps(seg_result_np)
                    )

                global_frame_idx += 1

                if (
                    args.visualization_interval > 0
                    and global_frame_idx % args.visualization_interval == 0
                ):
                    mask = np.array(seg_result) > 0
                    sup_img = superimpose_colored_mask(img, mask, (255, 0, 0))
                    sup_img.save(
                        join(hands_pkl_path, video_id, "jpgs", f"{frame_id}.jpg")
                    )
        except ToggleableException as ex:
    ...
    twohands_to_cb_model = init_segmentor(
        TWOHANDS_TO_CB_CONFIG_PATH, TWOHANDS_TO_CB_CHECKPOINT_PATH, device=args.device
    )
    ...
    for video_id, twohands_png_paths in twohands_png_paths_videos.items():
        ...
        for frame_idx, frame_id, twohands_png_img_path in tqdm(twohands_png_paths):
            try:
                ...
                pkl_out_path = join(
                    twohands_to_cb_pkl_path, video_id, "pkls", f"{frame_id}.pkl"
                )
                twohands_to_cb_png_paths_videos[video_id].append(
                    (frame_idx, frame_id, seg_result_png_path)
                )
                if isfile(pkl_out_path + ".zip"):
                    # need to temporarily save result as PNG (framework set up this way)
                ...
                # no image_width and image_height needed in output PKL since we can use the shape here
                seg_result = inference_segmentor(
                    twohands_to_cb_model, twohands_png_img_path
                )[0]
                seg_result_np = np.array(seg_result)
                ...
                with zipfile.ZipFile(
                    ...
                    if (
                        ...
                        sup_img_output_path = join(
                            twohands_to_cb_pkl_path, video_id, "jpgs", f"{frame_id}.jpg"
                        )
    ...
    twohands_cb_to_obj_model = init_segmentor(
        TWOHANDS_CB_TO_OBJ_CONFIG_PATH,
        TWOHANDS_CB_TO_OBJ_CHECKPOINT_PATH,
        device=args.device,
    )
    ...
    for video_id, twohands_to_cb_png_paths in twohands_to_cb_png_paths_videos.items():
        ...
        for frame_idx, frame_id, twohands_to_cb_png_img_path in tqdm(
            twohands_to_cb_png_paths
        ):
            try:
                frame_id = fmt_frame(video_id, frame_idx)
                ...
                # no image_width and image_height needed in output PKL since we can use the shape here
                seg_result = inference_segmentor(
                    twohands_cb_to_obj_model, twohands_to_cb_png_img_path
                )[0]
                seg_result_np = np.array(seg_result)
                ...
                if (
                    ...
                    sup_img.save(
                        join(obj_pkl_path, video_id, "jpgs", f"{frame_id}.jpg")
                    )
            except ToggleableException as ex:
                print(f"Error processing {frame_id}:", ex)
                continue
