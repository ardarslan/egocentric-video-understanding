...
def project_point_image(pt, pose_data: list, camera: dict):
...
def main(arg_dict=None):
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="/mnt/scratch/agavryushin/Thesis/data/EK_backprojected_points_vis/",
    )
    ...
    for video_id in args.generator_videos:
        ...
        sampled_idxs = np.random.choice(
            video_len, args.frame_sample_size, replace=False
        )
        for original_frame_idx in sampled_idxs:
            ...
            for depth_top_subdir in os.listdir(depth_top_path):
                ...
                original_frame_delta_idx = next(
                    (
                        int(spl[2:])
                        for spl in depth_top_subdir.split("_")
                ...
                aggregated_output_dirname = next(
                    filter(
                        lambda n: n.startswith("R"), os.listdir(depth_top_subdir_path)
                    ),
                    None,
                )
                frame_depth_path = join(
                    depth_top_subdir_path,
                ...
                if isfile(frame_depth_path):
                    ...
                    disparity_scaled = (disparity_data - disparity_data_q1) / (
                        disparity_data_q99 - disparity_data_q1
                    )
                    disparity_scaled = disparity_scaled**0.5

                    depth_img = visualize_depth(
                        disparity_data_blurred, disparity_data_q1, disparity_data_q99
                    )
                    ...
                    for point in point_data["points"]:
                        image_pt, pt_real_cam_space = project_point_image(
                            point[:3],
                            point_data["images"][original_frame_idx_str],
                            camera,
                        )
                        image_depth = pt_real_cam_space[2]
                        if not (
                            min(image_pt[0], image_pt[1]) >= 0
                            and image_pt[0] < cam_width
                            and image_pt[1] < cam_height
                        ):
                            # check if this is correct
                            continue

                        image_pt_rescaled = [
                            int(image_pt[0] / cam_width * img.width),
                            int(image_pt[1] / cam_height * img.height),
                        ]
                        image_pt_rescaled_depth = [
                            int(image_pt[0] / cam_width * depth_width),
                            int(image_pt[1] / cam_height * depth_height),
                        ]

                        if not (
                            0 <= image_pt_rescaled_depth[0] < depth_width
                            and 0 <= image_pt_rescaled_depth[1] < depth_height
                        ):
                            continue

                        if not (
                            0 <= image_pt_rescaled[0] < img.width
                            and 0 <= image_pt_rescaled[1] < img.height
                        ):
                            continue
                        ...
                        if (
                            np.linalg.norm(
                                img_np_blurred[
                                    image_pt_rescaled[1], image_pt_rescaled[0]
                                ][::-1]
                                - np.array(point[3:])
                            )
                            > args.max_color_diff_norm
                        ):
                            continue
                        ...
                        disparity = disparity_data_blurred[
                            image_pt_rescaled_depth[1], image_pt_rescaled_depth[0]
                        ]
                        if abs(disparity) < 1e-8:
                            continue

                        if (
                            disparity < disparity_data_q1
                            or disparity > disparity_data_q99
                        ):
                            continue
                        ...
                        pairs_xy_img_pre_proj_framewise[original_frame_idx].append(
                            image_pt_rescaled
                        )
                        pairs_xy_img_post_proj_framewise[original_frame_idx].append(
                            pt_real_cam_space
                        )
                        ...
                        draw.ellipse(
                            (
                                image_pt_rescaled[0] - ELLIPSE_RADIUS,
                                image_pt_rescaled[1] - ELLIPSE_RADIUS,
                                image_pt_rescaled[0] + ELLIPSE_RADIUS,
                                image_pt_rescaled[1] + ELLIPSE_RADIUS,
                            ),
                            fill=tuple(point[3:]),
                    ...
                    output_file_path = join(
                        output_dir_path, video_id, original_frame_idx_str
                    )
            ...
            for idx_1 in range(num_entries):
                ...
                for idx_2 in range(idx_1 + 1, num_entries):
                    ...
                    if abs(depth_pt_1 - depth_pt_2) <= args.max_s_depth_diff:
                        ...
                        if (
                            len(
                                set([0, np.nan]).intersection(
                                    set([real_x_dist, img_x_dist])
                                )
                            )
                            == 0
                        ):
        ...
        print(
            f"{s=} {t=} {len(s_list_x)=} s_x_mean={'%.4f' % s_x_mean} s_x_std={'%.4f' % s_x_std} s_y_mean={'%.4f' % s_y_mean} s_y_std={'%.4f' % s_y_std}"
        )
