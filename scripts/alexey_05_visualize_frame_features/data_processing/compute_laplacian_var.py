...
def main(arg_dict=None):
    ...
    parser.add_argument(
        "--output_path",
        type=str,
        default=f"./data/ek_variance_of_laplacian_{start_time}.pkl",
    )
        ...
        if args.hand_boxes_only:
            ...
            for hand, hand_bbox in hand_bbox_data[2][0].items():
                ...
                grayscale = cv2.cvtColor(
                    np.array(im_cropped_resized), cv2.COLOR_BGR2GRAY
                )
            ...
            if has_output_path:
                ...
                hand_bbox_data[2][0].update(
                    {
                        k + "_laplacian_var": v
                        for k, v in frame_out_dict["results"].items()
                    }
                )
            ...
            im_cropped_resized = pil_img.resize(
                (pil_img.width // 3, pil_img.height // 3), resample=Image.BILINEAR
            )
