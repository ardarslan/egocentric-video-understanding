    ...
    for annot_segments_sub in annot_segments:
        if len(annot_segments_sub) >= 3:  # need at least 3 coordinates
            polygon_segments = list(
                map(lambda c: (c[0] * x_ratio, c[1] * y_ratio), annot_segments_sub)
            )
...
def main(arg_dict=None):
    ...
    parser.add_argument(
        ...
        action="append",
        type=str,
        default=join(DATA_ROOT_PATH, "EK_VISOR"),
    )
    ...
    sparse_annotation_root_path = join(
        args.annotation_root_path, "GroundTruth-SparseAnnotations", "annotations"
    )
    dense_annotation_root_path = join(
        args.annotation_root_path, "Interpolations-DenseAnnotations"
    )
    for video_id in args.generator_videos:
        sparse_annotation_path_train_noext = join(
            sparse_annotation_root_path, "train", f"{video_id}"
        )
        sparse_annotation_path_val_noext = join(
            sparse_annotation_root_path, "val", f"{video_id}"
        )
        ...
        reader_original = VideoReader(
            get_video_path(video_id, use_original=True),
            get_extracted_frame_dir_path(video_id, use_original=True),
        )
        ...
        for sparse_annotation_path_candidate in [
            sparse_annotation_path_train_noext,
            ...
            for ext in [".json", ".zip"]:
                if isfile(sparse_annotation_path_candidate + ext):
                    ...
                    for annot_top_data_idx, annot_top_data in enumerate(
                        sparse_annotation_data["video_annotations"]
                    ):
                        name = annot_top_data["image"].get("name", "")
                        if name.startswith(video_id):
                            name = name[len(video_id) :]
                        original_frame_idx = next(
                            (
                                int(spl)
                                for spl in name.replace(".", "_").split("_")
                                if spl.isnumeric()
                            ),
                            None,
                        )
                        ...
                        for annot_subentry_idx, annot_subentry in enumerate(
                            annot_top_data["annotations"]
                        ):
                            annot_id = annot_subentry.get("id", "")
                            full_idx_id = (annot_top_data_idx, annot_subentry_idx)
                            if (
                                original_frame_idx
                                not in original_sparse_frame_idxs_to_annot_idxs
                            ):
                                original_sparse_frame_idxs_to_annot_idxs[
                                    original_frame_idx
                                ] = []
                            ...
                            original_contact_ids_to_annot_idxs[contact_key].append(
                                full_idx_id
                            )

                            if (
                                "in_contact_object" in annot_subentry
                                and (
                                    in_contact_object := annot_subentry[
                                        "in_contact_object"
                                    ]
                                )
                                is not None
                                and in_contact_object
                                not in [
                                    "",
                                    "hand-not-in-contact",
                                    "none-of-the-above",
                                ...
                                and all(
                                    (c in string.hexdigits for c in in_contact_object)
                                )
                            ):
                                if (
                                    original_frame_idx
                                    not in original_sparse_frame_idxs_to_contacted_objects
                                ):
                                    original_sparse_frame_idxs_to_contacted_objects[
                                        original_frame_idx
                                    ] = []
                    ...
                    for annot_top_data_idx, annot_top_data in enumerate(
                        ...
                        contact_mask_frame = np.zeros(
                            (reader.video_height, reader.video_width), dtype=bool
                        )
                        ...
                        for annot_subentry_idx, annot_subentry in enumerate(
                            ...
                            if (
                                len(annot_segments) > 0
                                and annot_id in original_sparse_contact_ids
                            ):
                                # test for intersection with contacted objects
                                dense_annot_img = Image.new(
                                    "L", (reader.video_width, reader.video_height), (0,)
                                )
                                dense_annot_img_draw = ImageDraw.Draw(dense_annot_img)
                                # here, we *do* need to rescale
                                draw_mask(
                                    dense_annot_img_draw,
                                    annot_segments,
                                    (255,),
                                    reader.video_width,
                                    reader.video_height,
                        ...
                        if contact_mask_frame.max():
                            hos_output_path_nozip = join(
                                HOS_DATA_DIR,
                                "ek-gt-sparse",
                                "object",
                                video_id,
                                "pkls",
                                f"{fmt_frame(video_id, original_frame_idx)}.pkl",
                            )
                            os.makedirs(dirname(hos_output_path_nozip), exist_ok=True)
                            with zipfile.ZipFile(
                                f"{hos_output_path_nozip}.zip",
                                "w",
                                zipfile.ZIP_DEFLATED,
                                False,
                            ) as zip_file:
                                zip_file.writestr(
                                    basename(hos_output_path_nozip),
                                    pickle.dumps(contact_mask_frame.astype(np.uint8)),
                                )
        ...
        dense_annotation_path_train_noext = join(
            dense_annotation_root_path, "train", f"{video_id}_interpolations"
        )
        dense_annotation_path_val_noext = join(
            dense_annotation_root_path, "val", f"{video_id}_interpolations"
        )
        dense_annotation_path = None

        for dense_annotation_path_candidate in [
            dense_annotation_path_train_noext,
        ...
        for annot_top_data_idx, annot_top_data in enumerate(
            dense_annotation_data["video_annotations"]
        ):
            ...
            for annot_subentry_idx, annot_subentry in enumerate(
                ...
                original_dense_frame_idxs_to_annot_idxs[original_frame_idx].append(
                    (annot_top_data_idx, annot_subentry_idx)
                )
        ...
        for frame_idx in range(reader.get_virtual_frame_count()):
            ...
            if original_frame_idx in original_dense_frame_idxs_to_annot_idxs:
                ...
                contacted_objects = original_sparse_frame_idxs_to_contacted_objects.get(
                    original_frame_idx, []
                )
                for contacted_object_id in contacted_objects:
                    contact_annot_idx = original_contact_ids_to_annot_idxs.get(
                        contacted_object_id
                    )
                    if contact_annot_idx is not None:
                        annot_subentry_data = sparse_annotation_data[
                            "video_annotations"
                        ][contact_annot_idx[0][0]]["annotations"][
                            contact_annot_idx[0][1]
                        ]
                ...
                for idx_pair in original_dense_frame_idxs_to_annot_idxs[
                    original_frame_idx
                ]:
                    annot = dense_annotation_data["video_annotations"][idx_pair[0]][
                        "annotations"
                    ][idx_pair[1]]
                    ...
                    annot_interpolation_id = dense_annotation_data["video_annotations"][
                        idx_pair[0]
                    ]["image"]["interpolation_start_frame"]
                    ...
                    if len(annot_segments) > 0:
                        ...
                        if found_contact_id is None:
                            ...
                            for (
                                contact_id,
                                contact_mask,
                            ) in frame_contacted_object_masks.items():
                                if (
                                    calculate_iou(contact_mask, dense_annot_mask)
                                    >= args.iou_matching_threshold_initial
                                ):
                                    found_contact_id = contact_id
                                    if (
                                        annot_interpolation_id
                                        not in dense_contact_store
                                    ):
                                    ...
                                    dense_contact_store[annot_interpolation_id][
                                        annot_name
                                    ][found_contact_id] = dense_annot_mask
                                    contact_mask_frame |= dense_annot_mask > 0
                                    break

                            if found_contact_id is None:
                                if (
                                    annot_stored_names_contacts := dense_contact_store.get(
                                        annot_interpolation_id
                                    )
                                ) is not None and (
                                    annot_name_stored_contacts := annot_stored_names_contacts.get(
                                        annot_name
                                    )
                                ) is not None:
                                    for (
                                        contact_id,
                                        contact_mask,
                                    ) in annot_name_stored_contacts.items():
                                        if (
                                            calculate_iou(
                                                contact_mask, dense_annot_mask
                                            )
                                            >= args.iou_matching_threshold_sequential
                        ...
                        color = (
                            *[
                                int(round(i))
                                for i in rng.uniform(low=0, high=255, size=3)
                            ],
                            128,
                        )
                        draw_mask(
                ...
                if not found_initial_contact:
                    ...
                    if abs(correction_factor - 1.0) > 1e-5:
                        frame_idx = round(
                            annot_interpolation_id_first_frames[annot_interpolation_id]
                            + correction_factor
                            * (
                                frame_idx
                                - annot_interpolation_id_first_frames[
                ...
                # keep this here
                annot_recent_frame = annot_interpolation_id_recent_frames.get(
                    annot_interpolation_id
                )
                ...
                os.makedirs(dirname(hos_output_path_nozip), exist_ok=True)
                with zipfile.ZipFile(
                    f"{hos_output_path_nozip}.zip", "w", zipfile.ZIP_DEFLATED, False
                ) as zip_file:
                    zip_file.writestr(
                        basename(hos_output_path_nozip),
                        pickle.dumps(contact_mask_frame.astype(np.uint8)),
                ...
                if (
                    annot_recent_frame is not None
                    and annot_recent_frame + 1 != frame_idx
                ):
                    for intermediate_idx in range(annot_recent_frame + 1, frame_idx):
                        hos_output_path_nozip = join(
                            ...
                            video_id,
                            "pkls",
                            f"{fmt_frame(video_id, intermediate_idx)}.pkl",
                        )
                        with zipfile.ZipFile(
                            f"{hos_output_path_nozip}.zip",
                            "w",
                ...
                if annots_found > 0:
                    ...
                    img_composite = Image.alpha_composite(img, img_polygons).convert(
                        "RGB"
                    )
                    os.makedirs(dirname(img_output_path), exist_ok=True)
                    img_composite.save(img_output_path)
                    print(
                        f"{img_output_path=} {annots_found=} {found_initial_contact=} {frame_idx=} {original_frame_idx=} {annot_interpolation_id_first_frames.get(annot_interpolation_id)=} {annot_interpolation_id=}"
                    )
                    pass
