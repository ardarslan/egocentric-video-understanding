...
def main(arg_dict=None):
    ...
    parser.add_argument(
        "--focus_threshold", type=float, default=DEFAULT_FOCUS_THRESHOLD
    )
    ...
    if args.tracking_mask:
        ...
        output_subdir = {"image": "full_image"}.get(
            args.image_version, args.image_version
        )
    else:
    ...
    for video_id in args.generator_videos:
        ...
        for frame_idx in tqdm((range_obj)):
            ...
            frame_outputs = []
            pkl_out_path = join(
                args.output_dir, output_subdir, video_id, f"{frame_id}.pkl"
            )
            ...
            if args.tracking_mask:
                tracking_mask_dir_path = CHANNEL_FRAME_PATH_FUNCTS["tracking_mask"](
                    video_id,
                    frame_idx,
                    frame_id,
                    args.image_version,
                ...
                tracking_mask_postprocessing_dir_path = CHANNEL_FRAME_PATH_FUNCTS[
                ...
                for dn in os.listdir(tracking_mask_dir_path):
                    ...
                    *_, mask_contours, mask_hierarchy_top = cv2.findContours(
                        track_mask_data["masks"],
                        cv2.RETR_EXTERNAL,
                    ...
                    for contour_idx, contour in enumerate(mask_contours):
                        ...
                        if (
                            min(
                                int(box_rescaled[2] - box_rescaled[0]),
                                int(box_rescaled[3] - box_rescaled[1]),
                            )
                            > 0
                        ):
                            ...
                            masks_out = [
                                {
                                    **mask,
                                    "segmentation": csr_matrix(mask["segmentation"]),
                                }
                                for mask in masks
                            ]
                            # cls 1: active object
                            frame_outputs.append(
                                {
                                    ...
                                    "contour_box_orig": box,
                                    "contour_box_rescaled": box_rescaled,
                                }
                            )
                ...
                masks_out = [
                    {**mask, "segmentation": csr_matrix(mask["segmentation"])}
                    for mask in masks
                ]
                frame_outputs.append(
                    {
                ...
                hand_pkl_zip_path = CHANNEL_FRAME_PATH_FUNCTS["hos_object"](
                    video_id, frame_idx, frame_id, args.hos_version
                )
            ...
            if focus_dict is not None:
                ...
                if focus_dict[frame_id] < focus_threshold:
                    ...
                    print(
                        f"{frame_id}: keeping (focus: {focus_dict[frame_id]=} >= {focus_threshold=})"
                    )
            ...
            with zipfile.ZipFile(
                ...
                zip_file.writestr(
                    os.path.basename(pkl_out_path), pickle.dumps(frame_outputs)
                )
