...
EK100_DATASET_VIDEO_LIST_PATH = (
    "/mnt/scratch/agavryushin/Thesis/data/ek_val_video_list.json"
)
...
def get_video_path(video_id, use_original=False):
    return join(
        EK100_PATH,
        "videos_original" if use_original else "videos_converted",
        f"{video_id}.MP4",
    )
...
def get_extracted_frame_dir_path(video_id, use_original=False):
    return join(
        EK100_PATH,
        "videos_extracted_original" if use_original else "videos_extracted",
        video_id,
    )
...
def get_dataset_csv(subset):
    if subset == "train":
        return pd.read_csv(join(EK100_PATH, "EPIC_100_train.csv")).sort_values(
            by=["video_id", "start_frame"]
        )
    elif subset == "val":
        return pd.read_csv(join(EK100_PATH, "EPIC_100_validation.csv")).sort_values(
            by=["video_id", "start_frame"]
        )
    else:
...
def get_action_recognition_frame_gen(
    subsets=["train", "val"],
    videos=None,
    add_deltas=[],
    max_width=None,
    ...
    if action_frames_only:
        for subset_name in subsets:
            subset = {"train": train_csv, "val": val_csv, "evaluation": val_csv}[
                subset_name
            ]
            current_reader = None
            ...
            for _, row in subset.iterrows():
                ...
                if video_id != current_video_id:
                    ...
                    current_reader = VideoReader(
                        get_video_path(video_id, use_original=use_original),
                        get_extracted_frame_dir_path(
                            video_id, use_original=use_original
                        ),
                        max_width,
                        max_height,
                ...
                for frame_idx in range_obj:
                    ...
                    try:
                        frame_img, original_frame_idx = current_reader.get_frame(
                            frame_idx, return_real_frame_idx=True
                        )
                    except ToggleableException as ex:
                        print(
                            f"Error reading frame {frame_idx} from video {video_id}:",
                            ex,
                        )
                        continue
        ...
        for video_id in videos:
            ...
            for virtual_frame_idx in range(current_reader.get_virtual_frame_count()):
                ...
                frame_img, original_frame_idx = current_reader.get_frame(
                    virtual_frame_idx, return_real_frame_idx=True
                )
                delta_imgs = {
                    delta: (
                        frame_img
