...
def process(process_idx, data):
    with torch.no_grad():
        ...
        sam_checkpoint = download_checkpoint(sam_checkpoint_url, checkpoint_dir, sam_checkpoint)
        xmem_checkpoint = download_checkpoint(xmem_checkpoint_url, checkpoint_dir, xmem_checkpoint)
        ...
        for (video_track_id, track_info) in video_track_infos_kv:
            ...
            tracking_mask_path = CHANNEL_FRAME_PATH_FUNCTS["tracking_mask"](video_id, None, None, args.image_version, args.hos_version, args.min_length, track_type)
            ...
            if args.use_gt_segment_boundaries:
                ...
                for row in video_section_rows.iterrows():
                    if row[1].loc["start_frame"] <= track_info["start"] < row[1].loc["stop_frame"]:
                        stop_at_frame = row[1].loc["stop_frame"]
                    elif last_stop_frame <= track_info["start"] < row[1].loc["start_frame"]:
            ...
            for frame_idx in (progress_bar := tqdm(range(track_info["start"], int(LARGE)))):
                try:
                    ...
                    if frame_idx >= track_info["end"] + 1:
                        if frame_idx - last_seen > (args.vanish_tolerance_frames_object if "obj" in track_type else args.vanish_tolerance_frames_hands):
                            break
                    
                    frame_id = fmt_frame(video_id, frame_idx)
                    
                    if args.image_version in [None, "image"]:
                        image = reader.get_frame(frame_idx).copy()  # avoid "negative stride" error
                    elif args.image_version.startswith("inpaint"):
                        inpainted_version = args.image_version.split("_", 1)[1]
                        path = CHANNEL_FRAME_PATH_FUNCTS["inpainted"](video_id, frame_idx, frame_id, inpainted_version)
                        if not isfile(path):
                    ...
                    if args.max_width is not None or args.max_height is not None:
                        ...
                        image_res.thumbnail((args.max_width or LARGE, args.max_height or LARGE))
                    ...
                    if frame_idx == track_info["start"]:
                        ...
                        scaled_box = np.array([box[0] * width_ratio, box[1] * height_ratio,
                                                box[2] * width_ratio, box[3] * height_ratio])

                        sam_controller.sam_controler.reset_image()
                        sam_controller.sam_controler.set_image(image)
                        masks, scores, logits =\
                            sam_controller.sam_controler.predictor.predict(box=scaled_box,
                                                                            multimask_output=False)
                        
                        template_mask = masks
                
                    masks, logits, painted_images =\
                        xmem.track(image, template_mask[0].astype(int) if template_mask is not None else None)
                    
                    found_mask = (masks > 0).max()
                    if found_mask:
                        last_seen = frame_idx

                    pkl_output_path = join(tracking_mask_path, track_id, f"{frame_id}__{track_id}.pkl")
                    output_dir = dirname(pkl_output_path)
                    os.makedirs(output_dir, exist_ok=True)

                    frame_outputs = {"timestamp": timestamp, "args": args, "masks": masks, "logits": logits, "type": track_info["type"],
                                    "image_width": image.shape[1], "image_height": image.shape[0]}

                    # Image.fromarray(painted_images).save(f"__test__{frame_id}.jpg")

                    with zipfile.ZipFile(pkl_output_path + ".zip", "w", zipfile.ZIP_DEFLATED, False) as zip_file:
                        zip_file.writestr(os.path.basename(pkl_output_path), pickle.dumps(frame_outputs))
    ...
    if args.num_jobs == 1:
        ...
        for job_idx in range(args.num_jobs):
            if job_idx == args.num_jobs-1:
                ...
                data_split.append(sorted_kv[(job_idx*split_share):((job_idx+1)*split_share)])
        with Pool(processes=args.num_jobs) as pool:
            pool.starmap(process, enumerate(zip([args] * args.num_jobs, [timestamp] * args.num_jobs, data_split)))
