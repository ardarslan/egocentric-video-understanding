        ...
        if isdir(video_depth_output_path):
            for subdir in os.listdir(video_depth_output_path):
                ...
                if isdir(subdir_output_path):
                    aggregated_output_dirname = next(filter(lambda n: n.startswith("R"), os.listdir(subdir_output_path)), None)
                    if (aggregated_output_dirname is not None 
                        and isdir(midas_output_path := join(subdir_output_path, aggregated_output_dirname))):
                        dirs_to_add.append(("/".join((video_id, subdir, aggregated_output_dirname)), midas_output_path))

                    depth_output_path = join(subdir_output_path, "depth_midas2", "depth")
                    if isdir(depth_output_path):
                        dirs_to_add.append(("/".join((video_id, subdir, "depth_midas2", "depth")), depth_output_path))

    os.makedirs(dirname(args.output_path), exist_ok=True)

    with zipfile.ZipFile(args.output_path, "w", zipfile.ZIP_DEFLATED, allowZip64=True) as zip_file:
