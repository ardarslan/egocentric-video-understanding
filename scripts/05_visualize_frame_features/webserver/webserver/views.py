...
class VideoDataReader(viewsets.ViewSet):
    ...
    def get_blip_answers(cls, request, clip_id, frame_id):
        if (
            ...
            cls.feature_name_feature_df_mapping["blip"] = pd.read_csv(
                os.path.join(
                    os.environ["SCRATCH"],
                    f"ego4d_data/v2/frame_features/{clip_id}/blip_vqa_features.tsv",
                ),
                sep="\t",
            )
        ...
        for blip_answer in blip_answers:
            current_answer = current_clip_frame_rows.loc[
                ...
                == cls.blip_vqa_feature_name_question_mapping[blip_answer],
                "answer",
            ].values[0]
