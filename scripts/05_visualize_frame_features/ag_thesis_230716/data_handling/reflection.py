...
def get_all_channels():
    ...
    return set(["hand_bbox", "tracking_bbox", "tracking_bbox_boundaries", "focus",
                ...
                *[f"tracking_mask_track=left-hand_image-version={i.replace('_', '-')}_min-length={l}_{v}"
                  for v in available_hos_versions for i in available_image_versions
...
def get_file_count_and_last_mod_timestamp(channel, video_id, start_frame=None, end_frame=None):
    if channel == "image":
        return -1, -1
    ...
    if channel.startswith("inpaint") or channel.startswith("depth"):
        path_funct_channel = "inpainted" if channel.startswith("inpaint") else "depth_postprocessing"
        spl = channel.split("_")
        version = "_".join(spl[1:])
        dir_path = CHANNEL_VIDEO_PATH_FUNCTS[path_funct_channel](video_id, version)
    elif (channel.startswith("hos") or "tracking_mask" in channel) and not channel.startswith("segmentation_mask_"):
        spl = channel.split("_")
        image_version = next((s.split("=", 1)[1].replace("-", "_") for s in spl if s.startswith("image-version=")), "image")
        hos_version = next((s for s in spl if s.startswith("threshold=") or s in ["egohos", "ek", "gt", "ek-gt", "ek-gt-sparse", "ek-gt-dense"]), None)  # float(s.split("=")[-1]) for ...
        track_type = next((s.split("=", 1)[1].replace("-", "_") for s in spl if s.startswith("track") and "=" in s), None)
        channel_name = "_".join([s for s in spl if "=" not in s and s not in ["egohos", "ek", "gt", "ek-gt", "ek-gt-sparse", "ek-gt-dense"]])  # must split ek_gt
        min_length = next((float(s.split("=", 1)[1]) for s in spl if s.startswith("min-length=")), None)

        if not channel.startswith("tracking_mask"):
            dir_path = CHANNEL_VIDEO_PATH_FUNCTS[channel_name](video_id, hos_version)
        else:
            if track_type is not None:
                dir_path = CHANNEL_VIDEO_PATH_FUNCTS[channel_name](video_id, image_version, hos_version, min_length, track_type)
            else:
                dir_path = CHANNEL_VIDEO_PATH_FUNCTS[channel_name](video_id, image_version, hos_version, min_length)
    ...
    if False:
        for root, dirs, files in os.walk(dir_path):
            for fn in files:
                frame_idx = next((int(ispl) for i in fn.split("_")[::-1] if (ispl := i.split(".")[0]).isnumeric()), None)
                if (not any((e is None for e in [frame_idx, start_frame, end_frame]))
                    and (start_frame or -1) > frame_idx or (end_frame or LARGE) < frame_idx):
                    continue
...
def get_availability_data(video_id, start_frame=0, end_frame=LARGE, request=None):
    def get_channel_cache_path(channel):
        return join(EK_DETAILED_AVAILABILITY_CACHE_DIR, video_id, f"{channel}__start={start_frame}__end={end_frame}.pkl")
    
    with open(FOCUS_DATA_PATH, "rb") as f:
        focus_dict = pickle.load(f)

    tracking_bbox_channels = ([strp for s in request.GET["tracking_channels"].split(",") if len(strp := s.strip()) > 0]
                              if request is not None and "tracking_channels" in request.GET else [])
    available_hos_versions = get_available_hos_versions()
    available_image_versions = get_available_image_versions()
    ...
    for channel in all_channels:
        ...
        file_count, last_mod_timestamp = get_file_count_and_last_mod_timestamp(channel, video_id, start_frame, end_frame)
    ...
    if True or len(all_channels_filtered.difference(handled_channels)) > 0:
        ...
        if "tracking_bbox_boundaries" not in handled_channels:
            av["tracking_bbox_boundaries"].append([0, reader.get_virtual_frame_count(), {}])

        for frame_idx in range(start_frame, min(end_frame + 1, reader.get_virtual_frame_count())):
                        ...
                        if "hand_bbox_list" in data:
                            for hand_data in data["hand_bbox_list"]:
                                for hand in ["left_hand", "right_hand"]:
                                    if hand in hand_data and type(hand_data[hand]) is np.ndarray:
                                        available += hand[0].upper()

                    if isfile(hand_mesh_vis_path):
                        available += "I"

                    if len(av[channel]) == 0 or av[channel][-1][1] != frame_idx or av[channel][-1][2] != available:
            ...
            if "hand_bbox" not in handled_channels:
                ...
                if isfile(hand_bbox_path):
                    ...
                    if len(av["hand_bbox"]) == 0 or av["hand_bbox"][-1][1] != frame_idx or av["hand_bbox"][-1][2] != available:
                    ...
                    if channel not in handled_channels:
                        ...
                        if isfile(hos_path):
                            ...
                            if version == "egohos":
                                ...
                                if "instances" not in hos_data or not hasattr(hos_data["instances"], pred_handsides):
                                    continue
                                
                                for cls, handside, mask, box in zip(hos_data["instances"].pred_classes,
            ...
            for segmentation_mask_version in available_segmentation_mask_versions:
                ...
                for object_bbox_version in available_object_bbox_versions:
                    ...
                    if channel not in handled_channels:
                        object_bbox_path = CHANNEL_FRAME_PATH_FUNCTS["object_bbox"](video_id, frame_idx, frame_id, object_bbox_version)
            ...
            if "tracking_bbox" not in handled_channels or "tracking_bbox_boundaries" not in handled_channels:
                ...
                if isfile(tracking_bbox_path):
                    ...
                    available = {k: list(v.keys()) for k, v in tracking_bbox_data["tracks"].items()}

                    if "tracking_bbox" not in handled_channels:
                        if len(av["tracking_bbox"]) == 0 or av["tracking_bbox"][-1][1] != frame_idx or av["tracking_bbox"][-1][2] != available:
                            av["tracking_bbox"].append([frame_idx, frame_idx+1, available])
                        else:
                    ...
                    for k, v in tracking_bbox_data["tracks"].items():
                        for bbox_id in list(v.keys()):
                            if bbox_id not in av["tracking_bbox_boundaries"][0][2]:
                                av["tracking_bbox_boundaries"][0][2][bbox_id] = [frame_idx, frame_idx+1]
                            else:
                                av["tracking_bbox_boundaries"][0][2][bbox_id][-1] = frame_idx+1
