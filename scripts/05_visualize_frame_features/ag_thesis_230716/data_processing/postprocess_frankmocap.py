...
def run_hand_mocap(args, bbox_detector, hand_mocap, visualizer, generator_videos=None):
    ...
    font = ImageFont.truetype("/mnt/scratch/agavryushin/Thesis/webserver/fonts/DejaVuSans.ttf", 25)
    ...
    for video_id in generator_videos:
        ...
        sample_vis_dir = f"/mnt/scratch/agavryushin/Thesis/data/vis_test_frankmocap/{video_id}/"
        os.makedirs(sample_vis_dir, exist_ok=True)
        
        reader = VideoReader(get_video_path(video_id), get_extracted_frame_dir_path(video_id),
                             assumed_fps=EK_ASSUMED_FPS, max_width=args.max_width, max_height=args.max_height)
        ...
        for frame_idx in tqdm(range_obj):
            ...
            for hand_side_idx, hand_side in enumerate(["left_hand", "right_hand"]):
                ...
                pred_joints_smpl[hand_side][frame_idx] = hand_side_data["pred_joints_smpl"]
                pred_joints_smpl_list[hand_side].append(hand_side_data["pred_joints_smpl"])
            ...
            for frame_idx in tqdm(range_obj):
                ...
                for neigh_idx_1 in range(len(neighbor_joints)):
                    for neigh_idx_2 in range(neigh_idx_1 + 1, len(neighbor_joints)):
                        dist = np.linalg.norm(neighbor_joints[neigh_idx_1][1] - neighbor_joints[neigh_idx_2][1])
                        if dist <= similarity_threshold:
                            union(neighbor_joints[neigh_idx_1][0], neighbor_joints[neigh_idx_2][0])
                
                # find largest component
                components_sorted = sorted(list(component_sizes.keys()), key=lambda k: component_sizes[k])
                ...
                if len(largest_component_members) == 0 or len(largest_component_members) == len(neighbor_joints):
                    continue
                largest_component_members.sort()

                if frame_idx not in largest_component_members or (len(components_sorted) >= 2 and 2 * components_sorted[-2] >= components_sorted[-1]):
                    ...
                    if False:
                        for pred_joint_idx in range(neighbor_joints[0].shape[-2]):
                            for coord_idx in range(neighbor_joints[0].shape[-1]):
                                spline = UnivariateSpline(x=[m - largest_component_members[0] for m in largest_component_members], y=[ neighbor_joints_dict[m][pred_joint_idx][coord_idx] for m in largest_component_members])
                                # regress this value:
        ...
        for hand_side in ["left_hand", "right_hand"]:
            ...
            betas_ewm[hand_side] = ewma_vectorized_2d(np.stack(betas[hand_side]), alpha, axis=1)
            thetas_ewm[hand_side] = ewma_vectorized_2d(np.stack(thetas[hand_side]), alpha, axis=1)
            cameras_ewm[hand_side] = ewma_vectorized_2d(np.stack(cameras[hand_side]), alpha, axis=1)
        ...
        for frame_seq_idx, frame_idx in tqdm(enumerate(range_obj)):
            ...
            for hand_side in ["left_hand", "right_hand"]:
                ...
                frame_id = fmt_frame(video_id, frame_idx)
                zip_path = CHANNEL_FRAME_PATH_FUNCTS["hand_mesh"](video_id, frame_idx, frame_id, args.version)
                ...
                if frame_handside_votes < 0:
                    try:
                        ...
                        if isinstance(pred_output_list_2, dict) and hand_side in pred_output_list_2:
                            pred_output_list_2[hand_side]["outlier"] = True

                        with zipfile.ZipFile(zip_path, "w", zipfile.ZIP_DEFLATED, False) as zip_file:
                            zip_file.writestr(os.path.basename(zip_path), pickle.dumps(result_data_2))
                    except ToggleableException as ex:
                ...
                if left_idx > 2 and hand_side == "left_hand":
                    ...
                    if frame_idx - last_frame_idx < 10:
                        ...
                        joint_change = np.linalg.norm(last_joints_smpl - this_joints_smpl)
                ...
                vert_smplcoord, hand_faces = __calc_hand_mesh(hand_side,
                                                              thetas_torch,
                                                              betas_torch,
                                                              hand_mocap.model_regressor.smplx,
                                                              device)
            ...
            if img_original is not None:
                ...
                pred_mesh_list = demo_utils.extract_mesh_from_output(result_data["pred_output_list"])
                ...
                demo_utils.save_res_img(dirname(output_path), basename(output_path), res_img)
...
def main():
    with torch.no_grad():
        options = DemoOptions()
        options.parser.add_argument("--generator_videos", action="append", type=str, default=None)
        options.parser.add_argument("--version", type=str, default=DEFAULT_HAND_MESH_VERSION)
        ...
        options.parser.add_argument("--ewm_span_rel_to_fps", type=float, default=(1.0/EK_ASSUMED_FPS))
        #  ewm_span_abs = round(EK_ASSUMED_FPS * args.ewm_span_rel_to_fps)
        args = options.parse()
        args.use_smplx = True
        ...
        # run
        run_hand_mocap(args, bbox_detector, hand_mocap, visualizer, args.generator_videos)
