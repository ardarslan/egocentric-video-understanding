...
class BLIPCaptioningFrameFeatureExtractor(FrameFeatureExtractor):
    def __init__(self, args):
        ...
        self.model = blip_decoder(
            pretrained=self.args.blip_captioning_model_file_path,
    ...
    def predictor_function(
        ...
        preprocessed_frames_batch = torch.vstack(preprocessed_frames_batch).to(
            self.args.device
        )
        del frames_batch
        gc.collect()
        ...
        predictions = []
        with torch.no_grad():
            image_captions = self.model.generate(
                preprocessed_frames_batch,
                sample=False,
                num_beams=3,
                max_length=20,
