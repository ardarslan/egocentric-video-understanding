...
class OFAFrameFeatureExtractor(FrameFeatureExtractor):
    def __init__(self, args):
        ...
        self.model = OFAModel.from_pretrained(args.ofa_model_file_path, use_cache=False).to(args.device)
        self.model.eval()
        self.questions = [
        ...
        self.question_input_ids_mapping = dict(
            (question, self.tokenizer([question], return_tensors="pt").input_ids.repeat(self.args.batch_size, 1).to(self.args.device)) for question in self.questions
        )
        self.patch_masks = torch.tensor([True], device=args.device).repeat(self.args.batch_size, 1)
    ...
    def predictor_function(self, frame_indices_batch: List[int], frames_batch: List[np.array]):
        ...
        for question in self.questions:
            ...
            data = self.model.generate(input_ids, patch_images=patch_images_batch, num_beams=5, no_repeat_ngram_size=3)
