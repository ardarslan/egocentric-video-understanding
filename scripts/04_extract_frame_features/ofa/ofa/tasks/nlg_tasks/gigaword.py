...
class GigawordTask(OFATask):
    ...
    def valid_step(self, sample, model, criterion):
        ...
        if self.cfg.eval_rouge:
            ...
            result = self.metric.compute(
                predictions=hyps, references=refs, use_agregator=False, use_stemmer=True
            )
            result_recall = {
                key: sum([item.recall for item in value]) * 100
                for key, value in result.items()
            }
            result_f1 = {
                key: sum([item.fmeasure for item in value]) * 100
                for key, value in result.items()
            }
    ...
    def reduce_metrics(self, logging_outputs, criterion):
        ...
        if sum_logs("_rouge_cnt") > 0:
            ...
            metrics.log_derived(
                "rouge1_recall",
                lambda x: x["_rouge1_recall_sum"].sum / x["_rouge_cnt"].sum,
            )
            metrics.log_derived(
                "rouge2_recall",
                lambda x: x["_rouge2_recall_sum"].sum / x["_rouge_cnt"].sum,
            )
            metrics.log_derived(
                "rougeL_recall",
                lambda x: x["_rougeL_recall_sum"].sum / x["_rouge_cnt"].sum,
            )
            metrics.log_derived(
                "rouge1_f1", lambda x: x["_rouge1_f1_sum"].sum / x["_rouge_cnt"].sum
            )
            metrics.log_derived(
                "rouge2_f1", lambda x: x["_rouge2_f1_sum"].sum / x["_rouge_cnt"].sum
            )
            metrics.log_derived(
                "rougeL_f1", lambda x: x["_rougeL_f1_sum"].sum / x["_rouge_cnt"].sum
            )
