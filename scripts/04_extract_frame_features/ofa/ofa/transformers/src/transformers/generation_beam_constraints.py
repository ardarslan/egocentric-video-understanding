...
class PhrasalConstraint(Constraint):
    ...
    def __init__(self, token_ids: List[int]):
        ...
        if not isinstance(token_ids, list) or len(token_ids) == 0:
            raise ValueError(
                f"`token_ids` has to be a non-emtpy list, but is {token_ids}."
            )
        if any(
            (not isinstance(token_id, int) or token_id < 0) for token_id in token_ids
        ):
            raise ValueError(
                f"Each list in `token_ids` has to be a list of positive integers, but is {token_ids}."
            )
...
class DisjunctiveConstraint(Constraint):
    ...
    def __init__(self, nested_token_ids: List[List[int]]):
        ...
        if not isinstance(nested_token_ids, list) or len(nested_token_ids) == 0:
            raise ValueError(
                f"`nested_token_ids` has to be a non-emtpy list, but is {nested_token_ids}."
            )
