...
PRETRAINED_POSITIONAL_EMBEDDINGS_SIZES = {
    "facebook/wav2vec2-lv-60-espeak-cv-ft": sys.maxsize
}
...
class Wav2Vec2PhonemeCTCTokenizer(PreTrainedTokenizer):
    ...
    def phonemize(self, text: str, phonemizer_lang: Optional[str] = None) -> str:
        ...
        phonemizer_lang = (
            phonemizer_lang if phonemizer_lang is not None else self.phonemizer_lang
        )
    ...
    def _add_tokens(
        ...
        for token in new_tokens:
            if not isinstance(token, str):
                raise ValueError(
                    f"Token {token} has to be of type string, but is "
                    f"of type {type(token)}."
                )
