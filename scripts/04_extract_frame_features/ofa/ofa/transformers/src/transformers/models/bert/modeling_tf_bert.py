...
class TFBertPreTrainingLoss:
    ...
    def hf_compute_loss(self, labels: tf.Tensor, logits: tf.Tensor) -> tf.Tensor:
        ...
        next_sentence_active_loss = tf.not_equal(
            tf.reshape(tensor=labels["next_sentence_label"], shape=(-1,)), -100
        )
        ...
        next_sentence_label = tf.boolean_mask(
            tensor=tf.reshape(tensor=labels["next_sentence_label"], shape=(-1,)),
            mask=next_sentence_active_loss,
        )
        ...
        masked_lm_loss = tf.reshape(
            tensor=masked_lm_loss, shape=(-1, shape_list(next_sentence_loss)[0])
        )
