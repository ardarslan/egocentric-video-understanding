    ...
    def call(
        ...
        **kwargs,
        ...
        mask, attn_mask = get_masks(
            slen, inputs["lengths"], self.causal, padding_mask=inputs["attention_mask"]
        )
        ...
        tensor = inputs["inputs_embeds"] + tf.gather(
            self.position_embeddings, inputs["position_ids"]
        )
...
class TFXLMForMultipleChoice(TFXLMPreTrainedModel, TFMultipleChoiceLoss):
    ...
    def call(
        ...
        **kwargs,
        ...
        flat_langs = (
            tf.reshape(inputs["langs"], (-1, seq_length))
            if inputs["langs"] is not None
            else None
        )
        flat_inputs_embeds = (
