...
class UnifiedVisualizationDemo(object):
    def __init__(self, unidet_cfg, instance_mode=ColorMode.IMAGE, parallel=False):
        """
        Args:
            unidet_cfg (CfgNode):
        ...
        if parallel:
            ...
            self.predictor = AsyncPredictor(cfg=unidet_cfg, num_gpus=num_gpu)
        else:
            self.predictor = DefaultPredictor(cfg=unidet_cfg)
...
class AsyncPredictor:
    ...
    def __init__(self, cfg, num_gpus: int = 1):
        ...
        for gpuid in range(max(num_gpus, 1)):
            unidet_cfg = unidet_cfg.clone()
            unidet_cfg.defrost()
            unidet_cfg.MODEL.DEVICE = "cuda:{}".format(gpuid) if num_gpus > 0 else "cpu"
            self.procs.append(
