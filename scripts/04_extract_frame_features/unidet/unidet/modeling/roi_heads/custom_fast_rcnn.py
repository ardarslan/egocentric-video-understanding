...
def _load_class_hierarchy(cfg):
    ...
    if cfg.MODEL.ROI_BOX_HEAD.HIERARCHY_IGNORE:
        ...
        hierarchy_data = json.load(open(cfg.MODEL.ROI_BOX_HEAD.HIERARCHY_PATH, "r"))
        ...
        catid2continous = {x["id"]: continousid.index(x["id"]) for x in categories}
        C = len(categories)
        is_parents = torch.zeros(
            (C + 1, C), device=torch.device(cfg.MODEL.DEVICE)
        ).float()
        is_chirlds = torch.zeros(
            (C + 1, C), device=torch.device(cfg.MODEL.DEVICE)
        ).float()
        for c in categories:
...
class CustomFastRCNNOutputLayers(FastRCNNOutputLayers):
    ...
    def sigmoid_cross_entropy_loss(
        ...
        weight = 1
        if (
            use_advanced_loss and (self.freq_weight is not None) and self.use_fed_loss
        ):  # fedloss
        ...
        if (
            use_advanced_loss
            and (self.hierarchy_weight is not None)
            and self.hierarchy_ignore
        ):
            if self.pos_parents:
