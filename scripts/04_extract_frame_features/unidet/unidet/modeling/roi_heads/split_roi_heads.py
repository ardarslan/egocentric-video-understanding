...
class MultiDatasetCascadeROIHeads(CustomCascadeROIHeads):
    ...
    def _init_box_head(self, cfg, input_shape):
        ...
        if self.unify_label_test:
            ...
            self.unified_num_class = len(
                set().union(*[label_map[d] for d in label_map])
            )
            ...
            self.label_map = {
                d: torch.cat(
                    [
                        self.label_map[d],
                        self.label_map[d].new_tensor([self.unified_num_class]),
                    ]
                )
                for d in label_map
            }
            self.class_count = (
                torch.zeros(self.unified_num_class + 1)
                .float()
    ...
    def _run_stage(self, features, proposals, stage, dataset_source):
        ...
        if self.unify_label_test and not self.training:
            ...
            if dataset_source in self.dataset_names:
                ...
                pred_class_logits = unified_score[
                    :, self.label_map[self.dataset_names[dataset_source]]
                ]
            else:
                pred_class_logits = unified_score
            # B x (#U + 1)
            ...
            if not self.training and (
                dataset_source == -1 or type(dataset_source) == type("")
            ):
                fg = torch.cat([x[:, :-1] for x in pred_class_logits], dim=1)
                bg = torch.cat([x[:, -1:] for x in pred_class_logits], dim=1).mean(
                    dim=1
                )
        ...
        if self.dump_cls_score:
            if not self.unify_label_test:
                pred_class_logits_all, _ = self.box_predictor[stage](box_features, -1)
