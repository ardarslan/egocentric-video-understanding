...
def fast_rcnn_inference_single_image(
    ...
    if not valid_mask.all():
        ...
        handsides = handsides[valid_mask]
    ...
    handsides = handsides[filter_inds[:, 0], :]
    # pdb.set_trace()
    contacts = contacts[filter_inds[:, 0], :]
    offsets = offsets[filter_inds[:, 0], :]
    # pdb.set_trace()
    ...
    handsides, contacts, offsets = handsides[keep], contacts[keep], offsets[keep]
...
class HOSFastRCNNOutputLayers(nn.Module):
    ...
    def losses(self, predictions, proposals):
        ...
        # print(f'**pred_contacts {contacts.shape} = {contacts[:10, :]}')
    ...
    #     '''
    #     loss_handside = cross_entropy(pred_handsides, gt_handsides, reduction='none')
    #     print('handside = ', loss_handside.shape, hand_mask.shape)
    #     print('loss_handside = ', loss_handside[:10])
    ...
    #     '''
    #     Only calculte contact loss for in-contact hands.
    #     '''
    #     loss_contact = cross_entropy(pred_contacts, gt_contacts, reduction='none')
    ...
    def predict_boxes(
        ...
        if not len(proposals):
            return []
        _, proposal_deltas, _, _, _ = predictions
        num_prop_per_image = [len(p) for p in proposals]
        proposal_boxes = cat([p.proposal_boxes.tensor for p in proposals], dim=0)
        predict_boxes = self.box2box_transform.apply_deltas(
            proposal_deltas,
    ...
    def predict_handsides_contacts_offsets(
        ...
        Returns:
            list[Tensor]:
                ...
                Element i has shape (Ri, K + 1), where Ri is the number of proposals for image i.
        """
        _, _, handsides, contacts, offsets = predictions
        num_inst_per_image = [len(p) for p in proposals]
