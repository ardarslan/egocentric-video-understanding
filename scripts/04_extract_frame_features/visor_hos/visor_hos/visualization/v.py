...
class Visualizer:
    ...
    def overlay_instances(
        ...
        *,
        ...
        # add handside, contact, offset, incontact_object_bbox from hand
        if handsides is not None:
            handsides = np.array(handsides)
            if num_instances:
                assert len(handsides) == num_instances
            else:
                num_instances = len(handsides)
        if contacts is not None:
            contacts = np.array(contacts)
            if num_instances:
                assert len(contacts) == num_instances
            else:
                num_instances = len(contacts)
        if offsets is not None:
            offsets = np.asarray(offsets)
            if num_instances:
                assert len(offsets) == num_instances
            else:
                num_instances = len(offsets)
        # if incontact_object_boxes is not None:
        #     incontact_object_boxes = self._convert_boxes(incontact_object_boxes)
        #     num_instances = len(incontact_object_boxes)
        ###
        ...
        if areas is not None:
            ...
            ###
            handsides = handsides[sorted_idxs] if handsides is not None else None
            contacts = contacts[sorted_idxs] if contacts is not None else None
            offsets = offsets[sorted_idxs] if offsets is not None else None
            # incontact_object_boxes = incontact_object_boxes[sorted_idxs] if incontact_object_boxes is not None else None
            #
    ...
    def draw_text(
        ...
        *,
        ...
        self.output.ax.text(
            ...
            y,
            text,
            size=font_size * self.output.scale * 3,
            family="sans-serif",
    ...
    def draw_box(self, box_coord, alpha=0.5, edge_color="g", line_style="-"):
        ...
        self.output.ax.add_patch(
            mpl.patches.Rectangle(
                ...
                linewidth=linewidth * self.output.scale * 2,
                alpha=alpha,
