...
def add_unidet_config(cfg):
    ...
    _C.MODEL.ROI_BOX_HEAD.USE_EQL_LOSS = (
        False  # Equalization loss described in https://arxiv.org/abs/2003.05176
    )
    ...
    _C.MODEL.ROI_BOX_HEAD.HIERARCHY_PATH = "datasets/oid/annotations/challenge-2019-label500-hierarchy-list.json"  # Hierarchical-loss for OpenImages
    _C.MODEL.ROI_BOX_HEAD.HIERARCHY_IGNORE = (
        False  # Ignore child classes when the annotation is an internal class
    )
    _C.MODEL.ROI_BOX_HEAD.HIERARCHY_POS_PARENTS = (
        False  # Set parent classes in the hierarchical tree as positive
    )
    _C.MODEL.ROI_BOX_HEAD.UNIFIED_MAP_BACK = (
        True  # Ignore out-of-dataset classes for retraining unified model
    )
    ...
    _C.MULTI_DATASET.UNIFY_LABEL_TEST = (
        False  # convert the partitioned model to a unified model at test time
    )
    ...
    _C.MULTI_DATASET.CAS_LAMBDA = 1.0  # Class aware sampling weight from https://arxiv.org/abs/2005.08455, not used in this project
    _C.MULTI_DATASET.UNIFIED_NOVEL_CLASSES_EVAL = (
        False  # zero-shot cross dataset evaluation
    )
