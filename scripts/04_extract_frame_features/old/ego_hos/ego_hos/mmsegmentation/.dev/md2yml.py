    ...
    with open(md_file, "r") as md:
        ...
        while i < len(lines):
            ...
            if len(line) == 0:
                ...
                is_backbone = True
                i += 1
            elif line[0] == "|" and (i + 1) < len(lines) and lines[i + 1][:3] == "| -":
                ...
                while j < len(lines) and lines[j][0] == "|":
                    ...
                    for fn in configs:
                        if fn in els[config_id]:
                            ...
                            model_name = fn[:-3]
                    fps = (
                        els[fps_id] if els[fps_id] != "-" and els[fps_id] != "" else -1
                    )
                    mem = (
                        els[mem_id].split("\\")[0]
                        if els[mem_id] != "-" and els[mem_id] != ""
                        else -1
                    )
                    ...
                    if fps != -1:
                        ...
                        model["Metadata"]["inference time (ms/im)"] = [
                            {
                                "value": round(1000 / float(fps), 2),
                                "hardware": "V100",
                                "backend": "PyTorch",
                                "batch size": 1,
                                "mode": "FP32" if "fp16" not in config else "FP16",
                                "resolution": f"({crop_size[0]},{crop_size[1]})",
                            }
                        ]
                    if mem != -1:
                    ...
                    if ms_id and els[ms_id] != "-" and els[ms_id] != "":
                        model["Results"][0]["Metrics"]["mIoU(ms+flip)"] = float(
                            els[ms_id]
                        )
                    models.append(model)
                    j += 1
                ...
                i += 1
    flag = (code_url is not None) and (paper_url is not None) and (repo_url is not None)
