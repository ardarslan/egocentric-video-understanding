...
parser.add_argument(
    "--hr_img_dir",
    default="/mnt/session_space/home/lingzzha/EgoHOS/data/train/image",
    type=str,
)
parser.add_argument(
    "--hr_lbl_dir",
    default="/mnt/session_space/home/lingzzha/EgoHOS/data/train/label",
    type=str,
)
parser.add_argument(
    "--img_dir",
    default="/mnt/session_space/home/lingzzha/EgoHOS/data/train/image_512",
    type=str,
)
parser.add_argument(
    "--lbl_dir",
    default="/mnt/session_space/home/lingzzha/EgoHOS/data/train/label_512",
    type=str,
)
parser.add_argument(
    "--lama_dir",
    default="/mnt/session_space/home/lingzzha/EgoHOS/data/train/lama_512",
    type=str,
)
parser.add_argument(
    "--lama_feat_dir",
    default="/mnt/session_space/home/lingzzha/EgoHOS/data/train/lama_512_feature",
    type=str,
)
parser.add_argument(
    "--aug_img_dir",
    default="/mnt/session_space/home/lingzzha/EgoHOS/data/train/image_ccda",
    type=str,
)
parser.add_argument(
    "--aug_lbl_dir",
    default="/mnt/session_space/home/lingzzha/EgoHOS/data/train/label_ccda",
    type=str,
)
args = parser.parse_args()

os.system("rm -rf " + args.aug_img_dir)
os.makedirs(args.aug_img_dir, exist_ok=True)
os.system("rm -rf " + args.aug_lbl_dir)
os.makedirs(args.aug_lbl_dir, exist_ok=True)
...
for file in tqdm(glob.glob(args.lama_feat_dir + "/*")):
    ...
    for aug_idx in range(args.aug_numbers):
        ...
        while not size_match:
            ...
            query_img = np.array(
                Image.open(os.path.join(args.img_dir, query_fname + ".jpg"))
            )
            query_lbl = np.array(
                Image.open(os.path.join(args.lbl_dir, query_fname + ".png"))
            )
            query_msk = np.zeros((query_lbl.shape))
            query_msk[query_lbl > 0] = 1
            query_msk = np.repeat(np.expand_dims(query_msk, 2), 3, 2)
            select_img = np.array(
                Image.open(os.path.join(args.lama_dir, select_fname + ".jpg"))
            )
        ...
        if args.composite_hr:
            hr_query_img = np.array(
                Image.open(os.path.join(args.hr_img_dir, query_fname + ".jpg"))
            )
            hr_query_lbl = np.array(
                Image.open(os.path.join(args.hr_lbl_dir, query_fname + ".png"))
            )
            hr_query_msk = np.zeros((hr_query_lbl.shape))
            hr_query_msk[hr_query_lbl > 0] = 1
            hr_query_msk = np.repeat(np.expand_dims(hr_query_msk, 2), 3, 2)
            hr_select_img = np.array(
                Image.open(os.path.join(args.lama_dir, select_fname + ".jpg")).resize(
                    (hr_query_img.shape[1], hr_query_img.shape[0])
                )
            )
        ...
        imsave(
            os.path.join(args.aug_img_dir, query_fname + "_" + str(aug_idx) + ".jpg"),
            new_img,
        )
        src_lbl_file = os.path.join(args.lbl_dir, query_fname + ".png")
        dst_lbl_file = os.path.join(
            args.aug_lbl_dir, query_fname + "_" + str(aug_idx) + ".png"
        )
