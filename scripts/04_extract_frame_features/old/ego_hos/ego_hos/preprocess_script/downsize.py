...
parser = argparse.ArgumentParser(
    description="resize image using nearest-neighbor upsampling"
)
parser.add_argument(
    "--img_dir",
...
for file in tqdm(glob.glob(args.img_dir + "/*")):
    ...
    if w > h:
        aspect_ratio = h / w
        if args.mask_tf:
            img_resize = Image.open(file).resize(
                (args.max_scale, int(args.max_scale * aspect_ratio)),
        ...
        if args.mask_tf:
            img_resize = Image.open(file).resize(
                (int(args.max_scale * aspect_ratio), args.max_scale),
                resample=Image.NEAREST,
            )
        else:
            img_resize = Image.open(file).resize(
