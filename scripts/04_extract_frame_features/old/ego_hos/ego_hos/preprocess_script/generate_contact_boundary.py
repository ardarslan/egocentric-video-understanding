...
for lbl_type in lbl_type_list:
    ...
    for file in tqdm(glob.glob(all_label_dir + "/*")):
        ...
        img = np.array(
            Image.open(os.path.join(all_img_dir, fname.split(".")[0] + ".jpg"))
        )
        ...
        hand_msk_dialte = cv2.dilate(
            hand_msk, kernel, iterations=int(img.shape[1] / 456)
        )
        ...
        imsave(
            os.path.join(contact_boundary_dir, fname), contact_boundary.astype(np.uint8)
        )
