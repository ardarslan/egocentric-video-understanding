...
class COLATask(OFATask):
    ...
    def valid_step(self, sample, model, criterion):
        ...
        with torch.no_grad():
            ...
            TP = sum(
                [
                    ref_dict.get(hyp, 0) if hyp == "yes" else 0
                    for ref_dict, hyp in zip(sample["ref_dict"], hyps)
                ]
            )
            FP = sum(
                [
                    1 - ref_dict.get(hyp, 0) if hyp == "yes" else 0
                    for ref_dict, hyp in zip(sample["ref_dict"], hyps)
                ]
            )
            TN = sum(
                [
                    ref_dict.get(hyp, 0) if hyp == "no" else 0
                    for ref_dict, hyp in zip(sample["ref_dict"], hyps)
                ]
            )
            FN = sum(
                [
                    1 - ref_dict.get(hyp, 0) if hyp == "no" else 0
                    for ref_dict, hyp in zip(sample["ref_dict"], hyps)
                ]
            )
