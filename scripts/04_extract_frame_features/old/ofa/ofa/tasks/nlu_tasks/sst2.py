...
from data.nlu_data.sst2_dataset import SST2Dataset
...
@dataclass
class SST2Config(OFAConfig):
    ans2label_dict: Optional[str] = field(
        default='{"negative": 0, "positive":1}',
...
@register_task("sst2", dataclass=SST2Config)
class SST2Task(OFATask):
    def __init__(self, cfg: SST2Config, src_dict, tgt_dict):
    ...
    def load_dataset(self, split, epoch=1, combine=False, **kwargs):
        ...
        self.datasets[split] = SST2Dataset(
            split,
            dataset,
            self.bpe,
            self.src_dict,
