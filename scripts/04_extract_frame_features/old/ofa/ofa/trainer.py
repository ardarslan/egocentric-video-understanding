...
class Trainer(object):
    ...
    def load_checkpoint(
        ...
        if bexists:
            ...
            try:
                if (
                    self.cfg.checkpoint.use_ema_weights_to_init_param
                    and "extra_state" in state
                    and "ema" in state["extra_state"]
                ):
                    logger.info(
                        "use_ema_weights_to_init_param = True, will use EMA weights in the ckpt to init the model param..."
                    )
                    ema_state_dict = (
                ...
                if not (
                    ...
                    and (
                        self.cfg.checkpoint.use_latest_weights_to_init_ema
                        or not (
                            "extra_state" in state and "ema" in state["extra_state"]
                        )
                    )
                ):
                    del state["model"]
