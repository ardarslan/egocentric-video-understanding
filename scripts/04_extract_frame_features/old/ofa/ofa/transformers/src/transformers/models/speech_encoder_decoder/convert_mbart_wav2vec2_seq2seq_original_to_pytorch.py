    ...
    if "adaptor" not in full_name:
        if "proj_ln" in full_name:
            ...
            if "bias" in name:
                ...
                logger.info(
                    f"Adapter proj layer norm bias was initialized from {full_name}."
                )
            if "weight" in name:
                assert (
            ...
            if "bias" in name:
                ...
                logger.info(
                    f"Adapter proj layer bias was initialized from {full_name}."
                )
            if "weight" in name:
                assert (
                ...
                logger.info(
                    f"Adapter proj layer weight was initialized from {full_name}."
                )
        ...
        if "bias" in name:
            ...
            logger.info(
                f"Adapter layer {layer_id} bias was initialized from {full_name}."
            )
        elif "weight" in name:
            assert (
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=None,
        type=str,
        help="Path to yaml file of fine-tuned model",
    )
    parser.add_argument(
        "--encoder_config_path",
    ...
    parser.add_argument(
        ...
        default=True,
        type=bool,
        help="whethere to add model adapter layers",
    )
    parser.add_argument(
        "--adapter_stride", default=2, type=int, help="stride of adapter layers"
    )
    parser.add_argument(
        "--adapter_kernel_size",
        default=3,
        type=int,
        help="kernel size of adapter layers",
    )
    parser.add_argument(
        "--encoder_output_dim", default=1024, type=int, help="encoder output dim"
    )
    parser.add_argument(
        "--start_token_id",
        default=250004,
        type=int,
        help="`decoder_start_token_id` of model config",
    )
