...
def bar_custom(current, total, width=80):
    print(
        "Downloading: %d%% [%d / %d] Ks"
        % (current / total * 100, current / 1000, total / 1000),
        end="\r",
    )
...
def check_need_manual_downalod(dl_folder, to_manually_download_urls):
    ...
    for url_record, instruction in to_manually_download_urls:
        ...
        if not os.path.exists(filename):
            print(
                f"{url} need to be download manually, please download it manually following {instruction}; and copy it to {filename}"
            )
...
def cut_wikitles(wiki_file, debug):
    ...
    if os.path.exists(to_file1) and os.path.exists(to_file2):
        debug and print(
            f"{wiki_file} already processed to {to_file1} and {to_file2}; so skip"
        )
        return wiki_file
...
def cut_tsv(file, debug):
    ...
    if os.path.exists(to_file1) and os.path.exists(to_file2):
        debug and print(
            f"{file} already processed to {to_file1} and {to_file2}; so skip"
        )
        return file
...
def convert_files_if_needed(extracted_foldrs, my_glob=my_glob, debug=False):
    return {
        url: list(
            sorted(
                set(convert_file_if_needed(f, debug))
                for f in sorted(set(my_glob(folder)))
            )
        )
...
def extracted_glob(extracted_folder, file_patterns, src, tgt, lang):
    def get_matching_pattern(file_pattern):
        ...
        file_pattern = re.sub(
            r"{src:(.*?)}", r"\1" if lang == src else "", file_pattern
        )
        file_pattern = re.sub(
            r"{tgt:(.*?)}", r"\1" if lang == tgt else "", file_pattern
        )
...
def all_extracted_files(split, src, tgt, extracted_folders, split_urls):
    ...
    return [
        f for url in split_urls for f in my_glob(extracted_folders[str(get_url(url))])
    ]
    ...
    for lang in [src, tgt]:
        ...
        for url in split_urls:
            ...
            for extracted_file in set(
                extracted_glob(
                    extracted_folders[str(url)], path_patterns, s_src, s_tgt, s_lang
                )
            ):
...
def lid_filter(split, src, tgt, from_folder, to_folder, debug=False):
    if not os.path.exists(LID_MODEL):
        call(
            f"wget -nc https://dl.fbaipublicfiles.com/fasttext/supervised-models/lid.176.bin -O {LID_MODEL}"
        )
    from_prefix = f"{from_folder}/{split}.{src}-{tgt}"
    to_prefix = f"{to_folder}/{split}.{src}-{tgt}"
    if os.path.exists(f"{from_prefix}.{src}") and os.path.exists(
        f"{from_prefix}.{tgt}"
    ):
...
def check_wmt_test_bleu(raw_folder, wmt_lang_pairs):
    ...
    for wmt, src_tgts in wmt_lang_pairs:
        for src_tgt in src_tgts:
            ...
            if bleu1 != 100.0:
                not_matchings.append(
                    f"{wmt}:{src_tgt} source side not matching: {test_src}"
                )
            bleu2 = run_eval_bleu(cmd2)
            if bleu2 != 100.0:
                not_matchings.append(
                    f"{wmt}:{src_tgt} target side not matching: {test_tgt}"
                )
...
def download_and_extract(
    ...
    to_be_manually_dowloaded = check_need_manual_downalod(
        dl_folder, to_manually_download_urls
    )
...
def download_czang16(download_to, username=None):
    ...
    if cmds and username is None:
        raise ValueError(
            "No czeng username is given; please register at http://ufal.mff.cuni.cz/czeng/czeng16 to obtain username to download"
        )
    for cmd in cmds:
        call(cmd)
...
def download_czeng17_script(download_to, extract_folder, debug=False):
    ...
    if not os.path.exists(script_path):
        wget.download(url, filename, bar=bar_custom)
        extract_to = extract_file(
            f"{download_to}/convert_czeng16_to_17.pl.zip",
            extract_folder,
            get_extract_name=get_extract_name,
            debug=debug,
        )
...
wmt13_es_en = DLDataset(
    ...
    valid_urls=[("http://www.statmt.org/wmt13/dev.tgz", "wmt13_dev.tgz")],
    test_urls=[("http://www.statmt.org/wmt13/test.tgz", "wmt13_test.tgz")],
    ...
    valid_files_patterns=[("dev/newstest2012.{lang}", ["es-en"])],
    test_files_patterns=[("test/newstest*.{lang}", ["es-en"])],
)
...
wmt14_de_fr_en = DLDataset(
    ...
    train_urls=[
        ...
        (
            ...
            "training-giga-fren.gz.tar",
        ),  # it is actuall a gz.tar
    ],
    valid_urls=[
    ...
    valid_files_patterns=[("dev/newstest2013.{lang}", ["fr-en", "de-en"])],
    test_files_patterns=[
        (
            "test-full/newstest*{src}{tgt}-{src:src}{tgt:ref}.{lang}",
            ["en-de", "de-en", "fr-en", "en-fr"],
        ),
    ],
)
...
wmt16_ro_en = DLDataset(
    ...
    train_urls=[
        (
            ...
            "wmt16_training-parallel-ep-v8.tgz",
        ),
        (
            "http://opus.nlpl.eu/download.php?f=SETIMES/v2/tmx/en-ro.tmx.gz",
            "en-ro.tmx.gz",
        ),
    ],
    valid_urls=[
    ...
    valid_files_patterns=[("dev/newsdev2016*{src}{tgt}*.{lang}", ["ro-en", "ro-en"])],
    test_files_patterns=[("test/newstest*{src}{tgt}*.{lang}", ["ro-en", "en-ro"])],
)
...
wmt17_fi_lv_tr_zh_en = DLDataset(
    ...
    train_urls=[
        (
            ...
            "wmt17_training-parallel-ep-v8.tgz",
        ),
        "http://data.statmt.org/wmt17/translation-task/training-parallel-nc-v12.tgz",
        "http://www.statmt.org/wmt15/wiki-titles.tgz",
        (
            "http://opus.nlpl.eu/download.php?f=SETIMES/v2/tmx/en-tr.tmx.gz",
            "en-tr.tmx.gz",
        ),
        (
    ...
    train_files_patterns=[
        ...
        (
            "training/*news-commentary-v12.{src}-{tgt}.{lang}",
            [
                "zh-en",
            ],
        ),
        ...
        (
            "wiki/fi-en/titles.{src}-{tgt}.{lang}",
            [
                "fi-en",
            ],
        ),
    ...
    valid_files_patterns=[
        (
            ...
            ["fi-en", "lv-en", "tr-en", "zh-en", "en-fi", "en-lv", "en-tr", "en-zh"],
        ),
        (
    ...
    test_files_patterns=[
        ...
        ("newstest2017-{src}{tgt}-{src:src}{tgt:ref}.{lang}", ["zh-en", "en-zh"]),
    ],
)
...
ru_en_yandex_instruction = (
    "Yandex Corpus download instruction at: https://translate.yandex.ru/corpus?lang=en"
)
...
wmt19_ru_gu_kk_lt = DLDataset(
    ...
    train_files_patterns=[
        ...
        (
            "*commoncrawl.{src}-{tgt}.{lang}",
            [
                "ru-en",
            ],
        ),
        (
            "*news-commentary-v14-wmt19.{tgt}-{src}.tsv.{lang}",
            [
                "kk-en",
            ],
        ),
        ...
        (
            "wikititles_v1_wikititles-v1.{src}-{tgt}.tsv.{lang}",
            ["ru-en", "kk-en", "lt-en", "gu-en"],
        ),
    ...
    valid_files_patterns=[
        (
            "dev/newsdev2019*{src}{tgt}-{src:src}{tgt:ref}.{lang}",
            ["gu-en", "kk-en", "lt-en"],
        ),
...
if __name__ == "__main__":
    ...
    if len(to_be_manually_dowloaded) > 0:
        print(
            "Missing files that need to be downloaded manually; stop the process now."
        )
        exit(-1)
    ...
    if len(not_matching) > 0:
        print(
            "the following datasets do not have matching test datasets:\n\t",
            "\n\t".join(not_matching),
        )
