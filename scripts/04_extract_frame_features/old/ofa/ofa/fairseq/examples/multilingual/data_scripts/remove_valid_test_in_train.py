...
def load_sentences(raw_data, split, direction):
    ...
    if os.path.exists(src_path) and os.path.exists(tgt_path):
        return [
            ...
            (tgt, open(tgt_path).read().splitlines()),
        ]
    else:
        return []
...
def check_train_all(raw_data, directions, all_test_data):
    ...
    for direction in directions:
        _, size = check_train_sentences(
            raw_data, direction, all_test_data, mess_up_train
        )
...
def count_train_in_other_set(mess_up_train):
    train_in_others = [
        (direction, s)
        for s, directions in mess_up_train.items()
        for direction in directions
    ]
    counts = {}
...
def remove_messed_up_sentences(
    raw_data, direction, mess_up_train, mess_up_train_pairs, corrected_langs
):
    split = "train"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--from-folder", required=True, type=str)
    parser.add_argument("--to-folder", required=True, type=str)
    parser.add_argument("--directions", default=None, type=str)
    ...
    if args.directions:
        ...
        directions = [
            os.path.split(file_path)[-1].split(".")[1] for file_path in raw_files
        ]
    ...
    mess_up_train, data_sizes = check_train_all(
        raw_data, directions, all_valid_test_data
    )
    ...
    # correct the dataset:
    all_test_pairs, mess_up_test_train_pairs = get_messed_up_test_pairs(
        "test", directions
    )
    all_valid_pairs, mess_up_valid_train_pairs = get_messed_up_test_pairs(
        "valid", directions
    )

    all_messed_pairs = set(mess_up_test_train_pairs.keys()).union(
        set(mess_up_valid_train_pairs.keys())
    )
    corrected_directions = set()

    real_data_situation = []
    for direction in directions:
        org_size, new_size = remove_messed_up_sentences(
            raw_data, direction, mess_up_train, all_messed_pairs, corrected_directions
        )
        if org_size == 0:
        ...
        real_data_situation.append(
            (
                ...
                org_size,
                org_size - new_size,
                (org_size - new_size) / org_size * 100,
            )
        )
    ...
    print(
        f"actual data clean summary (which can be different from the projected one because of duplications) in: {raw_data}/actual_clean_summary.tsv"
    )
