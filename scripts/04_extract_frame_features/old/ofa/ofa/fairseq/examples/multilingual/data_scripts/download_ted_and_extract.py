    ...
    def read_aligned_corpus(self, split_type="train"):
        ...
        if self.zero_shot:
            if split_type == "train":
                ...
                iterable = zip(
                    self.eval_lang_dict["source"], self.eval_lang_dict["target"]
                )

        elif self.bilingual:
            iterable = itertools.product(
                self.lang_dict["source"], self.lang_dict["target"]
            )
        ...
        for s_lang, t_lang in iterable:
            ...
            if self.corpus_type == "file":
                split_type_file_path = os.path.join(
                    self.corpus_path, "all_talks_{}.tsv".format(split_type)
                )
                s_list, t_list = self.read_from_single_file(
                    split_type_file_path, s_lang=s_lang, t_lang=t_lang
                )
...
def extra_english(corpus_path, split):
    ...
    with io.open(split_type_file_path, "r", encoding="utf8") as fp, io.open(
        output_split_type_file_path, "w", encoding="utf8"
    ) as fw:
        reader = csv.DictReader(fp, delimiter="\t", quoting=csv.QUOTE_NONE)
        for row in reader:
            line = row["en"]
...
def de_tok(tok_file, lang):
    ...
    cmd = "perl {detok_cmd} -l {lang} < {tok_file} > {de_tok_file}".format(
        detok_cmd=detok_cmd, tok_file=tok_file, de_tok_file=de_tok_file, lang=lang[:2]
    )
    call(cmd)
...
def extra_bitex(
    ...
    obj = MultiLingualAlignedCorpusReader(
        corpus_path=ted_data_path,
    ...
    obj.save_file(
        output_data_path + f"/train.{lsrc_lang}-{ltrg_lang}.{lsrc_lang}",
        split_type="train",
        data_type="source",
        lang=src_lang,
    )
    obj.save_file(
        output_data_path + f"/train.{lsrc_lang}-{ltrg_lang}.{ltrg_lang}",
        split_type="train",
        data_type="target",
        lang=trg_lang,
    )
    ...
    obj.save_file(
        output_data_path + f"/test.{lsrc_lang}-{ltrg_lang}.{lsrc_lang}",
        split_type="test",
        data_type="source",
        lang=src_lang,
    )
    obj.save_file(
        output_data_path + f"/test.{lsrc_lang}-{ltrg_lang}.{ltrg_lang}",
        split_type="test",
        data_type="target",
        lang=trg_lang,
    )
    ...
    obj.save_file(
        output_data_path + f"/valid.{lsrc_lang}-{ltrg_lang}.{lsrc_lang}",
        split_type="dev",
        data_type="source",
        lang=src_lang,
    )
    obj.save_file(
        output_data_path + f"/valid.{lsrc_lang}-{ltrg_lang}.{ltrg_lang}",
        split_type="dev",
        data_type="target",
        lang=trg_lang,
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--ted_data_path", type=str, default=WORKDIR_ROOT, required=False
    )
    parser.add_argument(
        "--direction-list",
        type=str,
