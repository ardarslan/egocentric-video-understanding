...
def eval_vqa_gen(task, generator, models, sample, **kwargs):
    if kwargs["beam_search_vqa_eval"]:
        hypos = task.inference_step(
            generator, models, sample, prefix_tokens=sample["prefix_tokens"]
        )
        results = []
        for i, sample_id in enumerate(sample["id"].tolist()):
            prefix_len = sample["prefix_tokens"][i].ne(1).sum().item()
            detok_hypo_str = decode_fn(
                hypos[i][0]["tokens"][prefix_len:], task.tgt_dict, task.bpe, generator
            )
            results.append(
                {"question_id": int(sample_id), "answer": detok_hypo_str.strip()}
            )
        scores = [
            ref_dict.get(result["answer"], 0)
            for ref_dict, result in zip(sample["ref_dict"], results)
        ]
    ...
    for valid_answers, valid_constraint_masks in zip(
        task.valid_answers_list, task.valid_constraint_masks_list
    ):
        ...
        valid_prev_output = data_utils.collate_tokens(valid_prev_items, pad_idx=pad).to(
            device
        )
        valid_constraint_masks = data_utils.collate_tokens(
            valid_constraint_mask_items, pad_idx=pad
        ).to(device)
    ...
    results = [
        {"question_id": int(id), "answer": hyp}
        for id, hyp in zip(sample["id"].tolist(), hyps)
    ]
...
def eval_snli_ve(task, generator, models, sample, **kwargs):
    ...
    hyps = [task.index2ans[predict_index] for predict_index in predicts]
    results = [
        {"uniq_id": id, "answer": hyp} for id, hyp in zip(sample["id"].tolist(), hyps)
    ]
    scores = [ref_dict.get(hyp, 0) for ref_dict, hyp in zip(sample["ref_dict"], hyps)]
    return results, scores
...
def eval_image_gen(task, generator, models, sample, **kwargs):
    ...
    caption = task.bpe.decode(
        task.tgt_dict.string([token for token in tokens if token >= 4])
    )[38:].replace("/", "")
    ...
    if task.cfg.gen_images_path:
        ...
        caption = task.bpe.decode(
            task.tgt_dict.string([token for token in caption_tokens if token >= 4])
        )[38:].replace("/", "")
        task.dump_images(
            sorted_hyps,
            text=caption,
            path=os.path.join(task.cfg.gen_images_path, "all_results"),
        )
        task.dump_images(
            sorted_hyps,
            text=caption,
            path=os.path.join(task.cfg.gen_images_path, "top1"),
            topk=1,
        )
...
def eval_glue(task, generator, models, sample, **kwargs):
    ...
    results = [
        {"hyp": hyp, "ref": ref_dict.keys()[0]}
        for hyp, ref_dict in zip(hyps, sample["ref_dict"])
    ]
    return results, None
...
def eval_gigaword(task, generator, models, sample, **kwargs):
    ...
    for i in range(len(gen_out)):
        hyp = (
            decode_fn(gen_out[i][0]["tokens"], task.tgt_dict, task.bpe, generator)
            .lower()
            .strip()
        )
...
def eval_image_classify(task, generator, models, sample, **kwargs):
    ...
    for valid_tgt, valid_prev_output, valid_constraint_masks in zip(
        ...
        valid_constraint_masks = valid_constraint_masks.repeat(batch_size, 1, 1).to(
            device
        )
        new_encoder_out = {}
        new_encoder_out["encoder_out"] = [
    ...
    if task.cfg._name == "image_gen":
        ...
        if cfg.distributed_training.distributed_world_size == 1 or dist.get_rank() == 0:
            ...
            output_path = os.path.join(
                ...
                "{}_predict.json".format(cfg.dataset.gen_subset),
            )
            gather_results = (
